...
CREATE OR REPLACE FUNCTION public.sync_event_attendee_count_from_follows()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    SET attendee_count = (
        ...
        WHERE ef.event_id = affected_event_id
    )
...
DROP TRIGGER IF EXISTS trg_event_follows_sync_attendee_count_insert ON public.event_follows;
CREATE TRIGGER trg_event_follows_sync_attendee_count_insert
    AFTER INSERT ON public.event_follows
    FOR EACH ROW EXECUTE FUNCTION public.sync_event_attendee_count_from_follows();

DROP TRIGGER IF EXISTS trg_event_follows_sync_attendee_count_delete ON public.event_follows;
CREATE TRIGGER trg_event_follows_sync_attendee_count_delete
...
CREATE POLICY "venue_follows_select" ON public.venue_follows
    FOR SELECT USING (
        ...
        OR EXISTS (
            ...
            WHERE v.id = venue_follows.venue_id
