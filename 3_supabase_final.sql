...
BEGIN
    ...
    SET avg_rating = (
        ...
        FROM public.venue_reviews WHERE venue_id = NEW.venue_id
    )
...
CREATE TRIGGER trg_venue_reviews_rating
    AFTER INSERT OR UPDATE ON public.venue_reviews
...
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "storage_event_photos_insert" ON storage.objects FOR INSERT WITH CHECK (
    bucket_id = 'event-photos' AND auth.uid() IS NOT NULL);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "storage_event_photos_delete" ON storage.objects FOR DELETE USING (
    bucket_id = 'event-photos' AND auth.uid()::text = SPLIT_PART(name, '/', 2));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
SELECT
    ...
    e.capacity, e.attendee_count, e.cover_gradient,
