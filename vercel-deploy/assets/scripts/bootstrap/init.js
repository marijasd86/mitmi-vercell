  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/sw.js')
        .then((reg) => {
          console.log('[Svita] SW registered:', reg.scope);
            ...
            newWorker.addEventListener('statechange', () => {
              if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                if (typeof _toast === 'function') {
                  _toast('Nova verzija je dostupna. Osvježi stranicu.', 'New version is available. Refresh the page.', 'info', 5000);
                } else if (typeof showToast === 'function') {
                  showToast('Nova verzija je dostupna. Osvježi stranicu.', 'info', 5000);
                }
              }
            });
          });
        })
        .catch((err) => console.warn('[Svita] SW registration failed:', err));
    });
  }
