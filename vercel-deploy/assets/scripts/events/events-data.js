...
function _mapDbDraftToUi(row = {}) {
  ...
  return {
    ...
    submittedByLabel: submitter?.display_name || submitter?.username || row.submitted_by || 'svita korisnik',
...
function _combinedEventCards() {
  const realEvents = Array.isArray(globalThis.REAL_EVENT_DATA) ? globalThis.REAL_EVENT_DATA : [];
  const localEvents = Array.isArray(globalThis.EVENT_DATA) ? globalThis.EVENT_DATA : [];
  const map = new Map();
  [...realEvents, ...localEvents.map(_mapDbEventToCard)].forEach(item => {
    const key = item.id || `${item.title}-${item.date || item.starts_at || ''}`;
    if (!map.has(key)) map.set(key, item);
  });
  return Array.from(map.values())
    .filter(item => _isEventUpcoming(item))
    .sort((a, b) => {
...
function _isEventUpcoming(item = null) {
  if (!item) return false;
  const startsAtRaw = item.starts_at || item.date || '';
  if (!startsAtRaw) return false;
  const startsAt = _parseEventDateLocal(startsAtRaw);
...
function _replaceRealEventCard(card = null) {
  if (!card?.id) return;
  const current = Array.isArray(globalThis.REAL_EVENT_DATA) ? globalThis.REAL_EVENT_DATA : [];
  globalThis.REAL_EVENT_DATA = [card, ...current.filter(item => item.id !== card.id)];
}
