  ...
  if (!_editingEventId) {
    ...
    if (contextHint) {
      contextHint.textContent = isManagedCreate
        ? _langText('Ako organizer već postoji, poveži ga ovde. Ako ne postoji, možeš upisati njegov naziv i mitmi će napraviti organizer u pripremi.', 'If the organizer already exists, connect it here. If not, you can type its name and mitmi will create an unclaimed organizer profile.')
    ...
    if (organizerEl && !organizerEl.value) {
      organizerEl.placeholder = isManagedCreate
        ? _langText('npr. Exit, Dom omladine ili naziv organizatora', 'e.g. Exit, Youth Center, or organizer name')
