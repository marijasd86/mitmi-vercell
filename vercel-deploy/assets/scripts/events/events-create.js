...
function _setCreateCoverPreview(coverUrl = '') {
  ...
  const focusYValue = document.getElementById('create-cover-focus-y');
  const focusControls = document.getElementById('create-cover-focus-controls');
  if (preview) {
    const cleanUrl = typeof _coverUrlWithoutFocus === 'function' ? _coverUrlWithoutFocus(coverUrl) : coverUrl;
    const pos = coverUrl
      ? `${Math.round(Math.max(0, Math.min(100, Number(globalThis._pendingEventCoverFocusX ?? 50))))}% ${Math.round(Math.max(0, Math.min(100, Number(globalThis._pendingEventCoverFocusY ?? 50))))}%`
      : '';
    preview.style.backgroundImage = cleanUrl ? `url(${cleanUrl})` : '';
    preview.style.backgroundSize = coverUrl ? 'cover' : '';
    preview.style.backgroundPosition = coverUrl ? pos : '';
  }
  if (empty) empty.style.display = coverUrl ? 'none' : '';
  if (clearBtn) clearBtn.style.display = coverUrl ? '' : 'none';
  if (focusYValue) focusYValue.textContent = `${Math.round(Math.max(0, Math.min(100, Number(globalThis._pendingEventCoverFocusY ?? 50))))}%`;
  if (focusControls) focusControls.style.display = coverUrl ? '' : 'none';
}
...
function resetCreateForm() {
  ...
  _pendingEventCover = '';
  globalThis._pendingEventCoverFocusX = 50;
  globalThis._pendingEventCoverFocusY = 82;
  ...
  const endDateEl = document.getElementById('create-end-date');
  const endTimeEl = document.getElementById('create-end-time');
  ...
  const endDateWrap = document.getElementById('create-end-date-wrap');
  const endTimeWrap = document.getElementById('create-end-time-wrap');
  ...
  const endDateLabelEl = document.getElementById('create-end-date-label');
  const endTimeLabelEl = document.getElementById('create-end-time-label');
  ...
  if (saveBtn) {
    ...
    saveBtn.textContent = _langText('Objavi plan', 'Publish plan');
  }
  ...
  if (endDateWrap) endDateWrap.style.display = '';
  if (endTimeWrap) endTimeWrap.style.display = '';
  ...
  if (endDateLabelEl) endDateLabelEl.textContent = _langText('Do datuma (opciono)', 'End date (optional)');
  if (endTimeLabelEl) endTimeLabelEl.textContent = _langText('Do vremena (opciono)', 'End time (optional)');
  if (locationLabel) locationLabel.textContent = _langText('Lokacija događaja (mesto održavanja)', 'Event location (venue)');
  ...
  if (contextLabel) contextLabel.textContent = _langText('Naziv organizatora (opciono)', 'Organizer name (optional)');
  if (contextHint) contextHint.textContent = _langText('Unesi organizatora (npr. pro.tok21). Lokaciju upiši u polje "Lokacija događaja", a adresu u polje "Adresa".', 'Enter the organizer name (e.g. pro.tok21). Use "Event location" for venue and "Address" for the exact address.');
  ...
  if (organizerEl) organizerEl.placeholder = _langText('npr. pro.tok21, SKCNS, Exit', 'e.g. pro.tok21, SKCNS, Exit');
...
function loadCreateForm() {
  ...
  if (!_editingEventId) {
    if (_createFlowMode === 'suggest') {
      ...
      if (reviewNoteCopy) reviewNoteCopy.textContent = _langText('Ovo nije direktna objava događaja. Pošalji osnovne podatke, a svita admin će proveriti detalje i objaviti događaj ako je sve u redu.', 'This is not a direct event publish flow. Send the basics and a svita admin will review the details and publish the event if everything looks good.');
      ...
      if (addressLabel) addressLabel.textContent = _langText('Adresa (opciono)', 'Address (optional)');
      if (contextLabel) contextLabel.textContent = _langText('Naziv organizatora (opciono)', 'Organizer name (optional)');
      if (contextHint) contextHint.textContent = _langText('Unesi naziv organizatora (npr. pro.tok21). Lokaciju i adresu upiši u posebna polja ispod.', 'Enter organizer name (e.g. pro.tok21). Fill location and address in their dedicated fields below.');
    ...
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.textContent = isManagedCreate ? _langText('Objavi događaj', 'Publish event') : _langText('Objavi plan', 'Publish plan');
    }
    if (contextEl) contextEl.style.display = 'none';
    if (categoryWrap) categoryWrap.style.display = '';
    if (tagsWrap) tagsWrap.style.display = '';
    ...
    if (contextLabel) contextLabel.textContent = isManagedCreate
      ? _langText('Naziv organizatora (opciono)', 'Organizer name (optional)')
      : _langText('Događaj ili mesto (opciono)', 'Event or place (optional)');
    if (contextHint) {
      contextHint.textContent = isManagedCreate
        ? _langText('Unesi organizatora ako ga znaš (npr. pro.tok21). Lokaciju i adresu upiši u zasebna polja. Ako organizator nije poznat, događaj može i bez toga da se objavi.', 'Add organizer name if you know it (e.g. pro.tok21). Keep location and address in their dedicated fields. If organizer is unknown, the event can still be published.')
    ...
    if (organizerEl && !organizerEl.value) {
      organizerEl.placeholder = isManagedCreate
        ...
        : _langText('npr. pro.tok21, SKCNS, Exit', 'e.g. pro.tok21, SKCNS, Exit');
    }
    if (suggestionsEl) {
  ...
  if (saveBtn) {
    saveBtn.disabled = false;
    saveBtn.textContent = _langText('Sačuvaj izmene', 'Save changes');
  }
  if (contextEl) contextEl.style.display = 'none';
  if (intentCard) intentCard.style.display = 'none';
  if (reviewNote) reviewNote.style.display = 'none';
  ...
  if (endDateEl) endDateEl.value = raw.ends_at ? String(raw.ends_at).slice(0, 10) : '';
  if (endTimeEl) {
    if (raw.ends_at) {
      const endDate = new Date(raw.ends_at);
      endTimeEl.value = Number.isNaN(endDate.getTime()) ? '' : `${String(endDate.getHours()).padStart(2, '0')}:${String(endDate.getMinutes()).padStart(2, '0')}`;
    } else {
      endTimeEl.value = '';
    }
  }
  ...
  {
    const focus = typeof _coverFocusFromUrl === 'function'
      ? _coverFocusFromUrl(_pendingEventCover, 50, 82)
      : { x: 50, y: 82 };
    globalThis._pendingEventCoverFocusX = focus.x;
    globalThis._pendingEventCoverFocusY = focus.y;
  }
...
async function openCreateEvent(eventId = null, mode = 'auto') {
  ...
  if (!card && _isSupabaseConfigured()) {
    try {
      ...
      fetchFailed = true;
      console.warn('[svita] openCreateEvent:', e.message);
    }
  }
  if (!card) {
    _editingEventId = null;
    ...
    showToast(
      fetchFailed
        ? _langText('Događaj trenutno nije učitan. Proveri internet i pokušaj ponovo.', 'The event is not loaded right now. Check your connection and try again.')
        : _langText('Događaj nije pronađen ili je uklonjen.', 'Event not found or removed.'),
      'error',
      2600
    );
    return;
...
function _draftSubmitterLabel() {
  const user = getUser() || {};
  return user.display_name || user.username || user.email || _langText('svita korisnik', 'svita user');
}
...
async function handleSuggestEventSubmit() {
  ...
  const endsAt = endDate ? `${endDate}T${(endTime || time || '23:59')}:00` : null;
  if (endsAt) {
  ...
  try {
    ...
    if (msg.includes('ends_at') && (msg.includes('column') || msg.includes('schema cache') || msg.includes('could not find'))) {
      try {
        ...
        showToast(_langText('Predlog je poslat bez "do" datuma jer baza još nema to polje.', 'Suggestion was sent without the end date because the database is not patched yet.'), 'info', 2600);
        return;
      } catch (retryErr) {
        console.warn('[svita] handleSuggestEventSubmit retry:', retryErr?.message || retryErr);
      }
    }
    console.warn('[svita] handleSuggestEventSubmit:', e.message);
...
function nudgeCreateCoverFocus(dx = 0, dy = 0) {
  if (!_pendingEventCover) return;
  const x = Math.max(0, Math.min(100, Number(globalThis._pendingEventCoverFocusX ?? 50) + Number(dx || 0)));
  const y = Math.max(0, Math.min(100, Number(globalThis._pendingEventCoverFocusY ?? 82) + Number(dy || 0)));
  globalThis._pendingEventCoverFocusX = x;
  globalThis._pendingEventCoverFocusY = y;
  _setCreateCoverPreview(_pendingEventCover);
}

function resetCreateCoverFocus() {
  if (!_pendingEventCover) return;
  globalThis._pendingEventCoverFocusX = 50;
  globalThis._pendingEventCoverFocusY = 82;
  _setCreateCoverPreview(_pendingEventCover);
