...
function loadCreateForm() {
  ...
  if (!_editingEventId) {
    ...
    if (contextLabel) contextLabel.textContent = isManagedCreate
      ? _langText('Organizer profil (opciono)', 'Organizer profile (optional)')
      : _langText('Događaj ili mesto (opciono)', 'Event or place (optional)');
    if (contextHint) {
      contextHint.textContent = isManagedCreate
        ? _langText('Dodaj organizer profil ako postoji. Ako ne postoji, događaj će se objaviti, a organizer možeš odmah da dodaš kroz admin organizer panel.', 'Add an organizer profile if it exists. If not, the event will still publish, and you can add the organizer right away from the admin organizer panel.')
