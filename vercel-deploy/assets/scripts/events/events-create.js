...
function resetCreateForm() {
  ...
  if (locationLabel) locationLabel.textContent = _langText('Mesto / venue', 'Place / venue');
...
function loadCreateForm() {
  ...
  if (!_editingEventId) {
    if (_createFlowMode === 'suggest') {
      ...
      if (dateLabelEl) dateLabelEl.textContent = _langText('Datum', 'Date');
      if (timeLabelEl) timeLabelEl.textContent = _langText('Vreme', 'Time');
      if (locationLabel) locationLabel.textContent = _langText('Mesto / venue', 'Venue / place');
      if (cityLabel) cityLabel.textContent = _langText('Grad', 'City');
    ...
    if (contextHint) {
      contextHint.textContent = isManagedCreate
        ? _langText('Ovaj događaj se objavljuje direktno na organizer profil.', 'This event will be published directly on the organizer profile.')
    ...
    if (organizerEl && !organizerEl.value && !isManagedCreate) {
      organizerEl.placeholder = _langText('npr. SKCNS, Dom omladine ili naziv događaja', 'e.g. SKCNS, Youth Center, or event name');
    }
    if (suggestionsEl) {
  ...
  if (!card && _isSupabaseConfigured()) {
    try {
      const rows = await _supaGet('events', {
        select: 'id,creator_id,venue_id,organizer_id,title,description,category,event_tags,city,location_name,starts_at,capacity,attendee_count,cover_url,is_published,is_cancelled,created_at',
        id: `eq.${eventId}`,
        limit: '1'
      });
