...
async function loadAdminOrganizersFromBackend(opts = {}) {
  ...
  try {
    ...
    const backendOrganizers = (Array.isArray(rows) ? rows : []).map((row) => ({
      ..._mapDbOrganizerToUi(row),
      createdAt: row?.created_at || '',
      activityUpcomingEvents: 0,
      activityTotalPublishedEvents: 0,
      activityScore: 0
    }));
    const organizerIds = backendOrganizers.map(item => item.id).filter(Boolean);
    if (organizerIds.length) {
      try {
        const encodedIds = organizerIds.join(',');
        const eventRows = await _supaGet('events', {
          select: 'organizer_id,starts_at',
          organizer_id: `in.(${encodedIds})`,
        ...
        (Array.isArray(eventRows) ? eventRows : []).forEach((row) => {
          ...
          const current = byOrganizer.get(organizerId) || { upcoming: 0, total: 0 };
          current.total += 1;
          const startsAt = row?.starts_at ? new Date(row.starts_at) : null;
          if (startsAt && startsAt >= now) current.upcoming += 1;
          byOrganizer.set(organizerId, current);
        });
        backendOrganizers.forEach((organizer) => {
          const stats = byOrganizer.get(organizer.id) || { upcoming: 0, total: 0 };
          organizer.activityUpcomingEvents = Number(stats.upcoming || 0);
          organizer.activityTotalPublishedEvents = Number(stats.total || 0);
          organizer.activityScore = organizer.activityUpcomingEvents * 100 + organizer.activityTotalPublishedEvents * 10;
        });
      } catch (e) {
        console.warn('[svita] loadAdminOrganizersFromBackend activity:', e.message);
      }
    }
