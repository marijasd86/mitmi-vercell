...
function _renderBrowseEventCard(event) {
  ...
  const coverStyle = event.cover_url ? ` style="${_coverInlineStyle(event.cover_url, 'center')}"` : '';
...
function renderBrowseEventsGrid() {
  ...
  grid.innerHTML = items.map(_renderBrowseEventCard).join('');
  _applyBrowseFilters();
  _syncBrowseLoadMoreUi();
}
...
async function loadPublishedEvents() {
  ...
  // Clear stale data first so old events do not flash before fresh fetch resolves.
  REAL_EVENT_DATA = [];
  globalThis.REAL_EVENT_DATA = [];
  globalThis._browseHomePreviewEvents = [];
...
function renderUskoroStrip() {
  ...
  strip.innerHTML = upcoming.map((ev, i) => {
    ...
    const delay = i * 0.07;
    const heroStyle = ev.cover_url ? `${_coverInlineStyle(ev.cover_url, 'center')};position:relative` : 'position:relative';
...
function renderBrowseHomeStrip() {
  ...
  strip.innerHTML = upcoming.map((ev, i) => {
    ...
    const coverStyle = ev.cover_url
      ? _coverInlineStyle(ev.cover_url, 'center')
      : '';
