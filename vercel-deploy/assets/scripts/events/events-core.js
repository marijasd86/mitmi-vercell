...
var EVENT_DATA = Array.isArray(globalThis.EVENT_DATA) ? globalThis.EVENT_DATA : [];
...
// Safety fallbacks: prevent hard runtime crashes if dependent scripts are delayed.
if (typeof globalThis._createOrganizerQueryValue !== 'function') {
  globalThis._createOrganizerQueryValue = function _createOrganizerQueryValueFallback() {
    return String(document.getElementById('create-organizer')?.value || '').trim();
  };
}
if (typeof globalThis.renderMyCreatedEvents !== 'function') {
  globalThis.renderMyCreatedEvents = function renderMyCreatedEventsFallback() {};
}
if (typeof globalThis.loadMyCreatedEvents !== 'function') {
  globalThis.loadMyCreatedEvents = async function loadMyCreatedEventsFallback() {
    return [];
  };
}
if (typeof globalThis._isLikelyAdminCatalogEvent !== 'function') {
  globalThis._isLikelyAdminCatalogEvent = function _isLikelyAdminCatalogEventFallback() {
    return false;
  };
}
if (typeof globalThis._syncEventCollections !== 'function') {
  globalThis._syncEventCollections = function _syncEventCollectionsFallback() {};
}
if (typeof globalThis._matchingOrganizersForQuery !== 'function') {
  globalThis._matchingOrganizersForQuery = function _matchingOrganizersForQueryFallback() {
    return [];
  };
}
if (typeof globalThis._mapPlanToInviteLike !== 'function') {
  globalThis._mapPlanToInviteLike = function _mapPlanToInviteLikeFallback(item = {}) {
    if (typeof globalThis._mapPlanToCardLike === 'function') {
      try { return globalThis._mapPlanToCardLike(item); } catch (e) {}
    }
    return item;
  };
}
...
const EVENT_CATEGORY_META = {
  ...
  scena_humor: { bucket: 'scena_humor', label: 'Humor i scena' },
  ...
  standup: { bucket: 'scena_humor', label: 'Humor i scena' },
  ...
  pozoriste: { bucket: 'kultura_umetnost', label: 'Kultura i umetnost' },
...
function _monthNameShort(date, lang = 'sr') {
  const idx = Number(date?.getMonth?.());
  if (!Number.isFinite(idx) || idx < 0 || idx > 11) return '';
  const sr = ['jan', 'feb', 'mar', 'apr', 'maj', 'jun', 'jul', 'avg', 'sep', 'okt', 'nov', 'dec'];
  const en = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  return (lang === 'en' ? en : sr)[idx] || '';
}

function _formatDayMonth(date, { withWeekday = false } = {}) {
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) return '';
  const lang = getCurrentLang() === 'en' ? 'en' : 'sr';
  const day = String(date.getDate());
  const month = _monthNameShort(date, lang);
  if (!withWeekday) return `${day}. ${month}`;
  const weekday = lang === 'en'
    ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][date.getDay()]
    : ['ned', 'pon', 'uto', 'sre', 'čet', 'pet', 'sub'][date.getDay()];
  return `${weekday} ${day}. ${month}`;
}

function _matchesEventCategoryFilter(filterCat = '', eventCat = '', eventBucket = '', eventTags = []) {
  const normalizedFilter = _eventVisualCategory(filterCat || '');
  if (!normalizedFilter || normalizedFilter === 'all') return true;
  if (_eventVisualCategory(eventCat || '') === normalizedFilter) return true;
  if (_eventVisualCategory(eventBucket || '') === normalizedFilter) return true;
  const tags = Array.isArray(eventTags)
    ? eventTags.map(item => String(item || '').trim().toLowerCase()).filter(Boolean)
    : [];
  // "Performans" se prikazuje i pod "Humor i scena" i pod "Kultura i umetnost".
  if (tags.includes('performans') && (normalizedFilter === 'scena_humor' || normalizedFilter === 'kultura_umetnost')) {
    return true;
  }
  return tags.some(tag => _eventVisualCategory(tag) === normalizedFilter);
}
...
function _formatEventMeta(event = {}) {
  ...
  let timeLabel = date && !Number.isNaN(date.getTime())
    ? date.toLocaleTimeString('sr-Latn', { hour: '2-digit', minute: '2-digit' })
    : '';
  if (endsAt) {
    const endDate = new Date(endsAt);
    if (!Number.isNaN(endDate.getTime())) {
      const sameDay = date && !Number.isNaN(date.getTime())
        && date.getFullYear() === endDate.getFullYear()
        && date.getMonth() === endDate.getMonth()
        && date.getDate() === endDate.getDate();
      const endTimeLabel = endDate.toLocaleTimeString('sr-Latn', { hour: '2-digit', minute: '2-digit' });
      timeLabel = sameDay
        ? `${timeLabel}–${endTimeLabel}`
        : `${timeLabel} → ${dateLabel(endsAt)} ${endTimeLabel}`;
    }
  }
...
function _formatEventDateTimeLine(event = {}) {
  ...
  const datePart = _formatDayMonth(parsed, { withWeekday: true }) || 'Termin uskoro';
  const timePart = parsed.toLocaleTimeString('sr-Latn', {
    hour: '2-digit',
    minute: '2-digit'
  });
  if (!endsAt) return `${datePart} · ${timePart}`;
  const endParsed = _parseEventDateLocal(endsAt);
  if (!endParsed) return `${datePart} · ${timePart}`;
  const sameDay = parsed.getFullYear() === endParsed.getFullYear()
    && parsed.getMonth() === endParsed.getMonth()
    && parsed.getDate() === endParsed.getDate();
  if (sameDay) {
    const endTimePart = endParsed.toLocaleTimeString('sr-Latn', {
      hour: '2-digit',
      minute: '2-digit'
    });
    return `${datePart} · ${timePart}–${endTimePart}`;
  }
  const endPart = _formatDayMonth(endParsed) || '';
...
function dateLabel(dateStr) {
  ...
  return _formatDayMonth(d) || _langText('Uskoro', 'Soon');
}
