...
async function saveDraftSupplement(draftId, opts = {}) {
  ...
  try {
    ...
    console.warn('[mitmi] saveDraftSupplement:', e.message);
...
async function approveDraft(draftId) {
  ...
  if (_isBackendDraft(draft) && _isSupabaseConfigured()) {
    try {
      ...
      if (!draft.startsAt) {
        const shouldUseFallbackTime = window.confirm(
          'Draft nema datum i vreme. Klikni OK da objavimo sa okvirnim terminom (sutra u 20:00), ili Cancel da prvo ručno dopuniš.'
        );
      ...
      console.warn('[mitmi] approveDraft preflight:', e.message);
