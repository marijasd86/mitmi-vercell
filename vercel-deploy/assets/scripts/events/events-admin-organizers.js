...
function _adminSanitizeOrganizerCreatePayload(payload = {}) {
...
async function _adminCreateOrganizerWithSchemaFallback(payload = {}) {
  let safePayload = _adminSanitizeOrganizerCreatePayload(_adminStripUnsupportedOrganizerColumns(payload));
  let lastError = null;
    ...
    try {
      ...
      if (!missingColumn || !Object.prototype.hasOwnProperty.call(safePayload, missingColumn)) {
        if (!triedMinimalPayload) {
          safePayload = _adminSanitizeOrganizerCreatePayload({
      ...
      safePayload = _adminSanitizeOrganizerCreatePayload(nextPayload);
    }
  }
