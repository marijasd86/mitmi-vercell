...
async function handleRegister() {
  ...
  if (!email || !email.includes('@')) { _authToast('Unesite validan email', 'error'); return; }
  if (!pass || pass.length < 8)       { _authToast('Lozinka mora imati min. 8 karaktera', 'error'); return; }
  if (!ageEl?.checked)                { _authToast('MITMI je trenutno namenjen samo punoletnim korisnicima (18+)', 'error'); return; }
  if (!legalEl?.checked)              { _authToast('Potvrdi da razumeš pravila, Uslove i Politiku privatnosti', 'error'); return; }
...
async function _submitVenueOnboarding() {
  ...
  if (!venueName) { _authToast('Unesi naziv organizacije', 'error'); return; }
