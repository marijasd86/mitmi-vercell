...
function doSearch(val) {
  ...
  if (typeof renderBrowseProfileSearchResults === 'function') {
    renderBrowseProfileSearchResults(val).catch?.(() => {});
  }
}
...
async function _persistEventCreateOrUpdate(input, payload, startsAt) {
  ...
  const runEventMutation = async (bodyPayload) => {
    if (_editingEventId) {
      return _supaFetch(`/rest/v1/events?id=eq.${_editingEventId}&creator_id=eq.${getUser()?.id}`, {
        method: 'PATCH',
        headers: { 'Prefer': 'return=representation' },
  ...
  let event;
  if (_editingEventId) {
    const updated = await submitEventMutation();
    ...
    const eventRes = await submitEventMutation();
