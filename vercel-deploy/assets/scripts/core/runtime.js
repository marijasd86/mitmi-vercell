...
async function _saveMyClaimedOrganizerProfile(fields = {}) {
  ...
  const cleanPayload = Object.fromEntries(
    Object.entries(payload).filter(([key, value]) => {
      if (key === 'name' || key === 'city') return true;
      return value !== undefined;
    })
  );
  const rows = await _patchMyOrganizerWithSchemaFallback(organizer.id, cleanPayload);
  _clearCache('organizer', getUser()?.id || 'guest');
  const updated = Array.isArray(rows) ? rows[0] : rows;
  return updated || { ...organizer, ...payload };
}

function _missingOrganizerUpdateColumnFromError(error) {
  const msg = String(error?.message || error?.data?.message || '');
  const match = msg.match(/Could not find the '([^']+)' column of 'organizers'/i);
  return match?.[1] || null;
}

async function _patchMyOrganizerWithSchemaFallback(organizerId, payload = {}) {
  let safePayload = _stripUnsupportedOrganizerColumns(payload);
  let lastError = null;
  let triedMinimalPayload = false;
  ...
  for (let i = 0; i < 6; i += 1) {
    try {
      ...
      lastError = error;
      const missingColumn = _missingOrganizerUpdateColumnFromError(error);
      ...
      if (!triedMinimalPayload) {
        safePayload = {
          name: safePayload?.name || 'Organizator',
          city: safePayload?.city || ''
        };
        triedMinimalPayload = true;
        continue;
      }
      throw error;
    }
...
async function _sendMsgToSupabase(text) {
  ...
  try {
    ...
    if (typeof sendPushEvent === 'function') {
      sendPushEvent('message.sent', {
        chat_id: _activeChatId,
        sender_id: getUser()?.id || null,
        content_preview: String(text || '').slice(0, 140)
      });
    }
...
function _normalizeBrowseCityLabel(value = '') {
  ...
  const normalized = raw
    ...
    .replace(/\s+/g, ' ')
    .trim();
  // "Srbija" i varijante (npr. "📍 Srbija", "cela Srbija") tretiramo kao bez city filtera.
  if (!normalized || normalized.includes('srbija') || normalized.includes('serbia')) return '';
  return normalized;
}
...
function _sanitizeOrganizerCreatePayload(payload = {}) {
  const required = new Set(['name', 'city', 'status']);
  const next = {};
  ...
  if (!next.name) next.name = 'Organizator u pripremi';
  if (!Object.prototype.hasOwnProperty.call(next, 'city')) next.city = '';
  if (!next.status) next.status = 'unclaimed';
  return next;
}

async function _insertOrganizerWithSchemaFallback(payload = {}) {
  let safePayload = _sanitizeOrganizerCreatePayload(_stripUnsupportedOrganizerColumns(payload));
  let lastError = null;
  let triedMinimalPayload = false;
  for (let i = 0; i < 6; i += 1) {
    try {
      ...
      if (!missingColumn || !Object.prototype.hasOwnProperty.call(safePayload, missingColumn)) {
        if (!triedMinimalPayload) {
          // Some projects have stricter RLS/constraints on optional fields.
          // Retry once with the smallest valid organizer payload.
          safePayload = _sanitizeOrganizerCreatePayload({
            name: safePayload?.name || payload?.name || 'Organizator u pripremi',
            city: safePayload?.city || payload?.city || '',
            status: safePayload?.status || payload?.status || 'unclaimed'
          });
          triedMinimalPayload = true;
          continue;
        }
      ...
      _persistUnsupportedOrganizerColumns(unsupported);
      const { [missingColumn]: _removed, ...nextPayload } = safePayload;
      safePayload = _sanitizeOrganizerCreatePayload(nextPayload);
    }
  }
