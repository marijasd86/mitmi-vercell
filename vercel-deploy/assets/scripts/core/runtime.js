...
function nav(id, opts = {}) {
  ...
  if (id === 'register') {
    const forcedType = opts.regType === 'venue' || requestedId === 'venue-register' ? 'venue' : 'user';
...
function syncSettingsPreferenceUI() {
  ...
  if (eventsPane && prefs.event_visibility === 'hidden') {
    ...
    if (activeEventsTab) {
      const fallbackTab = inviteTab && prefs.invite_visibility !== 'hidden'
        ? inviteTab
        : document.querySelector('#page-profile .prof-tabs .ptab:not([style*="display: none"])');
      const fallbackPane = fallbackTab?.id === 'profile-invites-tab'
...
async function closeMyAccount() {
  ...
  const confirmed = window.confirm(
    'Zatvaranjem naloga tvoj profil i aktivni sadržaj biće uklonjeni iz javnog prikaza. Ocene, utisci i istorija potrebni za bezbednost i reputaciju ostaće sačuvani. Da li želiš da nastaviš?'
  );
  if (!confirmed) return;

  const secondConfirm = window.confirm(
    'Ovo će sakriti tvoj profil, zatvoriti aktivne objave i odjaviti te iz aplikacije. Potvrdi zatvaranje naloga.'
  );
...
async function _getMyProfile() {
  ...
  const rows = await _supaGet('profiles', {
    id: `eq.${getUser()?.id}`,
    select: 'id,username,display_name,city,bio,avatar_url,gender,profile_visibility,role,status,avg_rating,rating_count',
    limit: '1'
  });
...
function _getSwipeData() {
  ...
  const source = (_swipeFiltered && _swipeFiltered.length)
    ? _swipeFiltered
    : [...eventSource, ...inviteSource].sort((a, b) => {
...
async function handleCreateInvite() {
  ...
  if (!title)    { showToast(isManagedFlow ? 'Unesi naziv događaja' : 'Unesi naslov traženja', 'error'); return; }
  ...
  if (btn) { btn.disabled = true; btn.textContent = isManagedFlow ? 'Objavljujemo događaj...' : 'Objavljujemo traženje...'; }

  try {
    const isManagedCreate = !_editingEventId && !_inviteEventId && isManagedFlow;
    const managedTarget = isManagedFlow ? await _resolveManagedEventTarget() : null;
    const managedOrganizer = isManagedFlow && managedTarget?.entity_type === 'organizer'
      ? managedTarget
      : null;
    const myVenue = isManagedFlow && managedTarget?.entity_type !== 'organizer'
      ? managedTarget
      : null;

    if (isManagedFlow && !managedTarget?.id) {
    ...
    const payload = {
      ...
      capacity:      spots,
      is_published:  shouldPublishEvent
    };
    if (isManagedFlow) {
    ...
    if (_editingEventId) {
      ...
      if (_createFlowMode === 'social') {
        await _createPlanRecord({
          ...
          spots,
          sourceUrl,
          eventId: selectedContextEventId || event.id || null,
    ...
    showToast(
      isManagedCreate ? 'Događaj je objavljen' : (isStandaloneSocialCreate ? 'Traženje društva je objavljeno' : 'Traženje društva je objavljeno'),
      'success'
    );
    resetCreateForm();
    await loadMyProfile();
