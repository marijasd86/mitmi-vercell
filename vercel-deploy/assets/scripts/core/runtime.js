...
function nav(id, opts = {}) {
  ...
  if (id === 'register') {
    ...
    setTimeout(() => selectRegType(forcedType), 0);
  }
  ...
  if (id === 'onboarding' && typeof resetUserOnboarding === 'function') {
    setTimeout(() => resetUserOnboarding(), 0);
  }
  if (id === 'login') {
    setTimeout(() => {
  ...
  try {
    await _supaFetch('/rest/v1/rpc/create_moderation_item', {
      ...
      body: JSON.stringify({
        ...
        p_reason: 'Admin pregled profila',
        p_source_type: 'admin',
        p_priority: 2,
        p_metadata: {
          label: label || 'Profil korisnika',
          context: 'quick_admin_profile_action'
        }
      })
    });
    showToast('Profil je dodat u moderation inbox', 'success', 1800);
    ...
    showToast('Dodavanje u moderation trenutno nije uspelo', 'error');
  }
}
...
function applyLang(l) {
  ...
  if (srCheck) srCheck.textContent = lang === 'sr' ? 'v' : '';
  if (enCheck) enCheck.textContent = lang === 'en' ? 'v' : '';
...
function toggleUserPref(key, btnEl) {
  ...
  if (key === 'notif_plans') {
    _saveUserPrefs({ notif_plans: nextValue, notif_invites: nextValue });
    return;
  }
...
async function cycleProfileVisibility() {
  ...
  try {
    ...
    console.warn('[mitmi] cycleProfileVisibility:', e?.message || e);
    showToast('Promena vidljivosti profila trenutno nije uspela', 'error', 2000);
  }
}
...
function cyclePlanVisibility() {
  ...
  _saveUserPrefs({ invite_visibility: nextValue, plan_visibility: nextValue });
  showToast(nextValue === 'hidden' ? 'Planovi su sakriveni sa profila' : 'Planovi su vidljivi na profilu', 'info', 1800);
}

const cycleInviteVisibility = cyclePlanVisibility;

function cycleEventVisibility() {
  const current = _getUserPrefs();
  const nextValue = current.event_visibility === 'profile' ? 'hidden' : 'profile';
  _saveUserPrefs({ event_visibility: nextValue });
  showToast(nextValue === 'hidden' ? 'Događaji su sakriveni sa profila' : 'Događaji su vidljivi na profilu', 'info', 1800);
}
...
async function closeMyAccount() {
  ...
  try {
    ...
    console.warn('[mitmi] closeMyAccount:', e.message);
...
async function _upsertMyProfile(profileFields = {}) {
  ...
  const payload = {
    ...
    profile_visibility: profileFields.profile_visibility || getUser()?.profile_visibility || 'registered',
...
function _normalizeBirthYear(value) {
  ...
  if (year < 1940 || year > currentYear - 16) return null;
  return year;
}
...
async function saveEditedProfile() {
  ...
  if (birthYearEl?.value?.trim() && !birthYear) {
    showToast('Unesi ispravno godište', 'error');
    return;
  }
...
function _normalizeVenueTarget(entity = null) {
  ...
  if (_isOrganizerEntity(entity)) {
    const publicType = entity.organizer_type || entity.venue_type || (entity.status === 'claimed' ? 'Organizer profil' : 'Organizer u pripremi');
    return {
      ...entity,
      id: entity.id,
      kind: 'organizer',
...
async function renderPublicVenueProfile(venue = null) {
  ...
  setText('vp-desc', data.description || (isOrganizer ? 'Opis organizatora još nije dodat.' : 'Opis mesta još nije dodat.'));
  ...
  if (publicMetaBox) {
    const metaRows = [
      data.public_address ? `📍 ${data.public_address}` : '',
      data.website_url ? `🌐 ${data.website_url}` : '',
      data.instagram_handle ? `📸 @${String(data.instagram_handle).replace(/^@+/, '')}` : '',
      data.public_contact_email ? `✉️ ${data.public_contact_email}` : '',
      data.public_contact_phone ? `☎️ ${data.public_contact_phone}` : ''
    ].filter(Boolean);
...
async function adminHideOrganizerProfile(target = null) {
  ...
  try {
    ...
    showToast('Organizer profil je sakriven', 'success', 1800);
    nav('admin-organizers', { noPageAnim: true, preserveScroll: true });
  } catch (e) {
    console.warn('[mitmi] adminHideOrganizerProfile:', e.message);
    showToast('Sakrivanje organizer profila trenutno nije uspelo', 'error');
  }
}
...
function _renderVenueDashboard(venue = null) {
  ...
  if (publicDetails) {
    const detailRows = [
      venue.public_address ? `📍 ${venue.public_address}` : '',
      venue.website_url ? `🌐 ${venue.website_url}` : '',
      venue.instagram_handle ? `📸 @${String(venue.instagram_handle).replace(/^@+/, '')}` : '',
      venue.public_contact_email ? `✉️ ${venue.public_contact_email}` : '',
      venue.public_contact_phone ? `☎️ ${venue.public_contact_phone}` : ''
    ].filter(Boolean);
...
async function saveEditedVenue() {
  ...
  const instagram_handle = instagramEl?.value?.trim() || '';
  ...
  if (!city) {
    showToast('Unesi grad organizatora ili mesta', 'error');
    return;
  }
...
const _browseState = { query: '', cat: 'all', date: '', range: '', city: '' };
...
function _applyBrowseFilters() {
  ...
  cards.forEach(card => {
    ...
    if (_browseState.cat && _browseState.cat !== 'all') {
      ...
      else if (_browseState.cat === 'izlasci') matchCat = cardBucket === 'kafa';
    ...
    const show = matchQ && matchCat && matchDate && matchCity;
    card.style.display = show ? '' : 'none';
    if (show) visible++;
  });
  if (empty) empty.style.display = visible === 0 ? 'block' : 'none';
...
async function _finalizeCreatedEvent(event, payload) {
...
async function handleCreatePlan() {
  ...
  try {
    ...
    const { payload, startsAt } = _buildEventPayload(input, createContext.managedOrganizer, createContext.myVenue);
    const event = await _persistEventCreateOrUpdate(input, payload, startsAt);
    const mapped = await _finalizeCreatedEvent(event, payload);
