...
function _getSwipeData() {
  ...
  return source.map(ev => ({
    ...
    going: ev.swipeType === 'invite'
      ...
      : (ev.going || `${(typeof _eventSpotsLabel === 'function' ? _eventSpotsLabel(ev.spots) : (ev.spots || 'Bez limita'))} · aktivna ekipa`),
    spots: ev.spots || '',
...
function loadSwipeCard(el, data) {
  ...
  if (isFront) {
    ...
    if (spEl)  {
      spEl.textContent = typeof _eventSpotsLabel === 'function'
        ? _eventSpotsLabel(data.spots)
