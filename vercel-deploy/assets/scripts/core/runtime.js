...
function _canLoadVenueDashboard(role = null) {
  const resolvedRole = role || getUser()?.user_metadata?.role || getUser()?.user_role || null;
  return resolvedRole === 'venue' || resolvedRole === 'admin';
}
...
async function renderPublicProfile(profile = null) {
  ...
  setText('other-profile-rating-count', Number(data.rating_count || 0) > 0 ? `(${Number(data.rating_count || 0)})` : 'Još nema ocena');
