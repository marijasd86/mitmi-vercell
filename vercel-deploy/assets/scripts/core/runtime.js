...
function _renderMyProfile(profile = null) {
  ...
  const ratingWrap = ratingEl?.closest('.prof-rating');
  ...
  if (ratingEl) ratingEl.textContent = ratingCount > 0 ? rating : 'Novo';
  if (ratingCountEl) ratingCountEl.textContent = ratingCount > 0 ? `(${ratingCount})` : 'Još nema ocena';
  if (ratingWrap) ratingWrap.classList.toggle('is-empty', ratingCount === 0);
