...
async function _getMyClaimedOrganizer() {
  ...
  try {
    let rows = [];
    try {
      rows = await _supaGet('organizers', {
        claimed_by_profile_id: `eq.${getUser()?.id}`,
        select: 'id,name,city,organizer_type,public_address,public_description,instagram_handle,website_url,public_contact_email,public_contact_phone,source_notes,cover_url,status,claimed_by_profile_id,created_at,updated_at',
...
function _getSwipeData() {
  ...
  const resolvedEvents = baseEvents.length ? baseEvents : fallbackEvents;
...
function _applyBrowseFilters() {
  ...
  cards.forEach(card => {
    ...
    // If event city is missing, do not hide it from the list.
    const matchCity = !_browseState.city || !cardCity || cardCity === _browseState.city;
    const show = matchQ && matchCat && matchTag && matchDate && matchCity;
    card.style.display = show ? '' : 'none';
    if (show) visible++;
  });
  if (empty) empty.style.display = visible === 0 ? 'block' : 'none';
  if (visible === 0 && cards.length > 0 && _browseState.city) {
    // Soft fallback: if city filter hides everything, show all cities instead of empty state.
    _browseState.city = '';
...
function _validateCreatePlanInput(input) {
  if (!getUser()?.id) {
    showToast('Sesija nije spremna. Osveži stranicu i prijavi se ponovo.', 'error', 2600);
    return false;
  }
  if (!input.title) {
  ...
  if (input.isManagedFlow && !input.location) {
    showToast('Unesi mesto održavanja ili okvirnu lokaciju', 'error');
    return false;
  }
  ...
  if (input.ticketPriceRsd != null && (!Number.isInteger(input.ticketPriceRsd) || input.ticketPriceRsd < 0)) {
    showToast('Unesi ispravnu cenu ulaznice', 'error');
    return false;
  }
  return true;
}
...
async function _resolveCreateManagedContext(input) {
  ...
  if (input.isManagedFlow) {
    try {
      ...
      console.warn('[mitmi] resolve managed target:', e?.message || e);
      managedTarget = null;
    }
  }
...
async function _createGhostOrganizerForManagedEvent(event, input, payload) {
  ...
  const organizerRows = await _supaFetch('/rest/v1/organizers', {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify({
      name: organizerName,
      ...
      cover_url: event.cover_url || payload.cover_url || null,
      public_address: input.address || input.location || null,
      source_notes: `Auto-created from admin publish flow for event ${event.id}`.trim(),
      status: 'unclaimed',
...
async function _persistEventCreateOrUpdate(input, payload, startsAt) {
  const shouldRetryWithoutOptionalColumns = (error) => {
    ...
    const missingOptionalColumn = ['ticket_price_rsd', 'public_address'].find((column) => (
...
async function _maybeAttachGhostOrganizerToManagedCreate(event, input, payload, createContext) {
  ...
  if (!String(input?.organizerText || '').trim()) return event;
  try {
    const linkedEvent = await _createGhostOrganizerForManagedEvent(event, input, payload);
    if (typeof loadAdminOrganizersFromBackend === 'function') {
      await loadAdminOrganizersFromBackend({ silent: true });
    }
    return linkedEvent || event;
  } catch (error) {
    console.warn('[mitmi] auto ghost organizer:', error.message);
    showToast('Događaj je objavljen, ali organizer profil nije automatski napravljen. Možeš ga dodati iz admin panela.', 'info', 3400);
    return event;
  }
}
...
async function submitCreateFromUi() {
  ...
  try {
    ...
    console.error('[mitmi] submitCreateFromUi:', e);
    ...
    if (normalizedMessage.includes('istekao')) {
      try {
        ...
        console.warn('[mitmi] submitCreateFromUi timeout recovery:', recoverErr?.message || recoverErr);
      }
    }
