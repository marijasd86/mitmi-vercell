...
async function _hydrateCurrentUserFromAuth() {
  ...
  try {
    ...
    console.warn('[mitmi] hydrate auth user failed:', e?.message || e);
  }
  return getUser() || null;
}
  ...
  try {
    ...
    if (type === 'signup') {
      _storePendingAuthNotice({
        type: 'info',
        message: 'Email je potvrđen. Nalog je spreman.'
      });
    ...
    console.warn('[mitmi] auth redirect parse failed:', e);
    return false;
  }
}
  ...
  try {
    ...
    if (!data.access_token) {
      if (regType === 'user') {
        try {
          ...
          console.warn('[mitmi] pending register profile storage unavailable:', e?.message || e);
        }
      }
      _storePendingAuthNotice({
        email,
    ...
    if (regType === 'user') {
      try {
        ...
        console.warn('[mitmi] register prefs storage unavailable:', e?.message || e);
      }
...
function goToUserOnboarding() {
  ...
  setTimeout(() => {
    ...
    try {
      ...
      console.warn('[mitmi] onboarding prefs storage read failed:', e?.message || e);
    }
    try {
      pending = JSON.parse(localStorage.getItem('mitmi_pending_register_profile') || '{}') || {};
    } catch (e) {
      pending = {};
      console.warn('[mitmi] pending register profile read failed:', e?.message || e);
    }
...
function prefillRegisterUserBasics() {
  ...
  try {
    ...
    pending = {};
    console.warn('[mitmi] register prefill storage read failed:', e?.message || e);
  }
...
async function saveOnboarding() {
  ...
  try {
    ...
    try {
      ...
      console.warn('[mitmi] onboarding prefs storage unavailable:', e?.message || e);
    }
    ['city-label','home-city-display','browse-city-label','browse-home-city-label'].forEach(id => {
      const el = document.getElementById(id);
