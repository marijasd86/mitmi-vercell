...
const SUPA_URL  = 'https://odjnyhjgsqhoynxpjcmk.supabase.co';  // Project URL
const SUPA_ANON = 'sb_publishable_qh0Q4aIgpAUEW7rK0fZQXw_yeDba9HL'; // publishable public key
...
const REGISTER_COPY = {
  user: {
    title: { sr: 'Kreiraj nalog', en: 'Create account' },
    sub: { sr: 'Nađi društvo za sledeći izlazak', en: 'Find people for your next outing' },
    cta: { sr: 'Napravi nalog za druženje', en: 'Create social account' },
    hint: { sr: 'Prvo napravi profil, pa biramo grad, interesovanja i tempo izlazaka.', en: 'Create the account first, then we set your city, interests, and social rhythm.' },
    rolePill: { sr: 'Praviš nalog za druženje', en: 'Creating a social account' },
    emailPlaceholder: { sr: 'tvoj@email.com', en: 'you@email.com' }
  },
  venue: {
    title: { sr: 'Registruj organizatora događaja', en: 'Register an event organizer' },
    sub: { sr: 'Objavljuj događaje i pošalji zahtev za pregled profila', en: 'Publish events and submit your profile for review' },
    cta: { sr: 'Nastavi kao organizator', en: 'Continue as organizer' },
    hint: { sr: 'Posle naloga sledi onboarding za organizatora događaja i ručni pregled profila u roku od 48h.', en: 'After signup, organizer onboarding follows with manual profile review within 48 hours.' },
    rolePill: { sr: 'Praviš nalog za organizatora', en: 'Creating an organizer account' },
    emailPlaceholder: { sr: 'organizator@email.com', en: 'organizer@email.com' }
  }
};

function _copyText(value) {
  if (value && typeof value === 'object' && (value.sr || value.en)) {
    return _langText(value.sr || value.en || '', value.en || value.sr || '');
  }
  return String(value || '');
}

const PENDING_AUTH_NOTICE_KEY = 'mitmi_pending_auth_notice';
const POST_AUTH_TARGET_KEY = 'mitmi_post_auth_target';
...
function _storePostAuthTarget(target = '') {
  try {
    if (!target) {
      sessionStorage.removeItem(POST_AUTH_TARGET_KEY);
      return;
    }
    sessionStorage.setItem(POST_AUTH_TARGET_KEY, target);
  } catch (e) {}
}

function _consumePostAuthTarget() {
  try {
    const target = sessionStorage.getItem(POST_AUTH_TARGET_KEY) || '';
    if (target) sessionStorage.removeItem(POST_AUTH_TARGET_KEY);
    return target;
  } catch (e) {
    return '';
  }
...
function _renderAuthNotice(targetId, payload = null) {
  ...
  box.style.background = isError ? '#FCEAEA' : '#F8EBE7';
  box.style.borderColor = isError ? '#E6B3B3' : '#D9B3A8';
  box.style.color = isError ? '#8E2F2F' : '#7E3B2B';
...
function _authToast(message, type = 'info', duration = 2200) {
  ...
  console[type === 'error' ? 'error' : 'log']('[svita auth]', message);
}
...
function _friendlyAuthError(msg = '', fallback = 'Greška pri registraciji') {
  ...
  if (lower.includes('email not confirmed') || (lower.includes('email') && lower.includes('not confirmed'))) {
    return 'Email još nije potvrđen. Otvori inbox i potvrdi nalog, pa pokušaj ponovo.';
  }
...
function _clearRegisterFieldErrors() {
  ['reg-email', 'reg-pass', 'reg-city', 'reg-birth-year', 'reg-interests', 'reg-tempo-block', 'reg-legal-block'].forEach(clearFieldError);
}
...
function validateRegisterForm() {
  ...
  if (!ageChecked) {
    return { ok:false, errorField:'reg-legal-block', errorMessage:'SVITA je trenutno namenjen samo punoletnim korisnicima (18+).' };
  }
  if (!legalChecked) {
  ...
  if (regType === 'user') {
    ...
    const birthYear = typeof _normalizeBirthYear === 'function'
      ? _normalizeBirthYear(document.getElementById('reg-birth-year')?.value?.trim() || '')
      : null;
    const interests = _selectedRegisterInterests();
    const socialTempo = document.querySelector('input[name="reg-social-tempo"]:checked')?.value || '';
    if (!city) {
      return { ok:false, errorField:'reg-city', errorMessage:'Izaberi ili upiši grad.' };
    }
    if (!birthYear) {
      return { ok:false, errorField:'reg-birth-year', errorMessage:'Unesi ispravno godište.' };
    }
    if (interests.length < 3) {
      return { ok:false, errorField:'reg-interests', errorMessage:'Izaberi bar 3 interesovanja.' };
    }
    if (!socialTempo) {
      return { ok:false, errorField:'reg-tempo-block', errorMessage:'Izaberi tempo izlazaka.' };
    }
  }
...
function _goToRoleHome(role) {
  ...
  if (target.page === 'home' && typeof openUnifiedHub === 'function') {
    openUnifiedHub('home', target.navIndex);
  } else {
    nav(target.page);
...
function _needsUserOnboarding(profile = null) {
  ...
  return !(city && birthYear && interests.length >= 3 && socialTempo);
}

function _routeAfterAuth(profile = null, role = null) {
  const resolvedRole = role || _resolveCurrentRole(profile);
  if (_needsUserOnboarding(profile)) {
    goToUserOnboarding();
    return;
  }
  _goToRoleHome(resolvedRole);
}
...
async function _hydrateCurrentUserFromAuth() {
  if (!isLoggedIn() || getUser()?.id || !_isSupabaseConfigured()) {
    return getUser() || null;
  }
  try {
    const user = await _supaFetch('/auth/v1/user', { method: 'GET' });
    if (user?.id) {
      _saveSession({
        ..._session,
        user
    ...
    console.warn('[svita] hydrate auth user failed:', e?.message || e);
  }
  return getUser() || null;
}
...
function _handleSupabaseAuthFailure(status = 401, data = {}, path = '') {
  ...
  console.warn('[svita] auth guard redirect:', status, path, data?.message || data?.error_description || '');
...
async function _restoreSession() {
  try {
    ...
    if (s.expires_at && Date.now() / 1000 > s.expires_at - 60) {
      ...
      if (s.refresh_token) {
        _session = _normalizeSession(s);
        _refreshToken(s.refresh_token).catch(() => _clearSession());
        return true;
      }
      _clearSession();
      return false;
...
async function _refreshToken(refreshToken) {
  ...
  try {
    ...
    console.warn('[svita] Token refresh failed:', e.message);
    _clearSession();
...
async function _refreshTokenOnce() {
  ...
  _refreshInFlight = _refreshToken(_session.refresh_token)
    .catch(() => false)
    .finally(() => {
...
async function _supaFetch(path, opts = {}) {
  ...
  const _doFetch = async () => {
    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
      throw Object.assign(new Error('Offline'), { status: 0, data: {}, code: 'OFFLINE', isNetwork: true });
    }
    ...
    let res;
    try {
      res = await fetch(`${SUPA_URL}${path}`, { ...opts, headers });
    } catch (networkErr) {
  ...
  let data;
  try {
    ({ res, data } = await _doFetch());
  } catch (error) {
    if (typeof _notifyNetworkIssue === 'function') {
      _notifyNetworkIssue(error?.code === 'OFFLINE' ? 'offline' : 'network');
    }
    throw error;
  }

  // Ako je token istekao, pokušaj jedan refresh + retry pre auth redirecta.
  if (
    res.status === 401 &&
    _shouldHandleAuthFailure(path, opts) &&
    _session?.refresh_token &&
    !String(path || '').startsWith('/auth/v1/')
  ) {
    const refreshed = await _refreshTokenOnce();
    if (refreshed) {
      ({ res, data } = await _doFetch());
    }
  }
  ...
  if (!res.ok) {
    if (res.status >= 500 && typeof _notifyNetworkIssue === 'function') {
      _notifyNetworkIssue('server');
    }
    // 401 => sesija/token problem, 403 => dozvole (RLS/policy), ne rušimo sesiju na 403.
    if (res.status === 401 && _shouldHandleAuthFailure(path, opts) && isLoggedIn()) {
...
async function handleLogin() {
  ...
  try {
    ...
    try {
      ...
      console.warn('[svita] login profile bootstrap warning:', profileErr);
    }
    _renderAuthNotice('login-auth-notice', null);
    _authToast('Dobrodošli! 👋', 'success');
    _routeAfterAuth(profile, role);
    if (typeof syncPushSubscriptionIfPermitted === 'function') {
      syncPushSubscriptionIfPermitted().catch(() => {});
    }
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'Prijavi se';
...
function _consumeAuthRedirectSession() {
  try {
    ...
    if (type === 'signup') {
      _storePendingAuthNotice({
        type: 'info',
        message: 'Dobrodošla u svitu. Email je potvrđen i nalog je spreman.'
      });
    } else if (type === 'recovery') {
      _storePendingAuthNotice({
        type: 'info',
        message: 'Link je potvrđen. Sada možeš da postaviš novu lozinku.'
      });
      _storePostAuthTarget('password-security');
    }
    ...
    console.warn('[svita] auth redirect parse failed:', e);
    return false;
  }
}
...
async function handleEmailMagicLogin() {
  ...
  try {
    ...
    _renderAuthNotice('login-auth-notice', {
      type: 'info',
      message: 'Poslali smo ti svita link za prijavu. Otvori poruku i vrati se u aplikaciju.'
    });
...
async function handleForgotPassword() {
  ...
  try {
    ...
    _renderAuthNotice('login-auth-notice', {
      type: 'info',
      message: 'Poslali smo svita email za reset lozinke. Proveri inbox i spam folder.'
    });
...
function _setRegisterLoading(isLoading) {
  ...
  if (!isLoading) {
    btn.textContent = _copyText(REGISTER_COPY[regType]?.cta) || _langText('Nastavi', 'Continue');
    return;
  }
  btn.textContent = regType === 'venue'
    ? _langText('Otvaram onboarding...', 'Opening onboarding...')
    : _langText('Kreiram nalog...', 'Creating account...');
}
...
function _syncRegisterUI() {
  ...
  const btn = document.getElementById('reg-submit-btn');
  const userBasics = document.getElementById('register-user-basics');
  if (title) title.textContent = _copyText(copy.title);
  if (sub) sub.textContent = _copyText(copy.sub);
  if (hint) hint.textContent = _copyText(copy.hint);
  if (rolePill) rolePill.textContent = _copyText(copy.rolePill || '');
  if (email) email.placeholder = _copyText(copy.emailPlaceholder);
  if (btn && !btn.disabled) btn.textContent = _copyText(copy.cta);
  if (userBasics) userBasics.style.display = regType === 'user' ? 'grid' : 'none';
}
...
async function handleRegister() {
  ...
  const email = emailEl?.value.trim();
  const pass  = passEl?.value;
  const registerCity = document.getElementById('reg-city')?.value?.trim() || '';
  const registerBirthYear = typeof _normalizeBirthYear === 'function'
    ? _normalizeBirthYear(document.getElementById('reg-birth-year')?.value?.trim() || '')
    : null;
  const registerInterests = _selectedRegisterInterests();
  const registerSocialTempo = document.querySelector('input[name="reg-social-tempo"]:checked')?.value || '';
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          ...(regType === 'user' ? {
            ...
            social_tempo: registerSocialTempo
          } : {})
        }
      })
    });
    if (!data.access_token) {
      if (regType === 'user') {
        try {
          localStorage.setItem('mitmi_pending_register_profile', JSON.stringify({
            city: registerCity,
            birth_year: registerBirthYear,
            interests: registerInterests,
            social_tempo: registerSocialTempo
          ...
          console.warn('[svita] pending register profile storage unavailable:', e?.message || e);
        }
      }
      _storePendingAuthNotice({
        email,
        type: 'info',
        message: 'Nalog je kreiran. Proveri email, potvrdi registraciju i vrati se u svitu.'
      });
      _setRegisterLoading(false);
      nav('login');
      return;
    ...
    if (regType === 'user') {
      try {
        ...
        localStorage.removeItem('mitmi_pending_register_profile');
      } catch (e) {
        console.warn('[svita] register prefs storage unavailable:', e?.message || e);
      }
      ['city-label','home-city-display','browse-city-label','browse-home-city-label'].forEach(id => {
        const el = document.getElementById(id);
        if (el && registerCity) el.textContent = registerCity;
      });
      if (typeof _normalizeBrowseCityLabel === 'function' && registerCity) {
        _browseState.city = _normalizeBrowseCityLabel(registerCity);
      }
    }
    ...
    if (regType === 'venue') {
      ...
      _routeAfterAuth(profile, 'user');
    }
    if (typeof syncPushSubscriptionIfPermitted === 'function') {
      syncPushSubscriptionIfPermitted().catch(() => {});
    }
  } catch(e) {
    console.error('[svita] register:', e);
    _setRegisterLoading(false);
    const msg = e.data?.error_description || e.message;
...
async function handleLogout() {
  try {
    ...
    if (typeof _onesignalLogoutUser === 'function') {
      _onesignalLogoutUser();
    }
    _clearSession();
    nav('landing');
    const bottomNav = document.getElementById('bottom-nav');
...
function _selectedRegisterInterests() {
  if (typeof _collectInterestSelection === 'function') {
    return _collectInterestSelection('reg-interest-grid');
  }
  return Array.from(document.querySelectorAll('#reg-interest-grid .int-item.sel'))
    .map(item => item.getAttribute('data-interest'))
    .filter(Boolean);
}

function updateRegisterInterestCount() {
  const countEl = document.getElementById('reg-interest-count');
  if (countEl) countEl.textContent = _langText(`Izabrano: ${_selectedRegisterInterests().length}`, `Selected: ${_selectedRegisterInterests().length}`);
}

function _applyRegisterPreferenceState(interests = [], socialTempo = 'povremeno') {
  if (typeof _setInterestSelectionState === 'function') {
    _setInterestSelectionState('reg-interest-grid', interests);
  }
  const tempoValue = typeof _normalizeSocialTempo === 'function' ? _normalizeSocialTempo(socialTempo) : (socialTempo || 'povremeno');
  document.querySelectorAll('input[name="reg-social-tempo"]').forEach(input => {
    input.checked = input.value === tempoValue;
  });
  updateRegisterInterestCount();
}

function toggleRegisterInterest(el) {
  if (!el) return;
  el.classList.toggle('sel');
  updateRegisterInterestCount();
  clearFieldError('reg-interests');
}
...
function _applyOnboardingPreferenceState(interests = [], socialTempo = 'povremeno') {
  if (typeof _setInterestSelectionState === 'function') {
    _setInterestSelectionState('ob-interest-grid', interests);
  }
  const tempo = typeof _normalizeSocialTempo === 'function'
    ? _normalizeSocialTempo(socialTempo)
    : (socialTempo || 'povremeno');
  const tempoInput = document.querySelector(`input[name="ob-social-tempo"][value="${tempo}"]`);
  if (tempoInput) tempoInput.checked = true;
...
function resetUserOnboarding() {
  ...
  ['ob-city', 'ob-birth-year', 'ob-interests', 'ob-tempo-block'].forEach(clearFieldError);
  updateOnboardingInterestCount();
}
...
function goToUserOnboarding() {
  ...
  setTimeout(() => {
    ...
    const birthYearEl = document.getElementById('ob-birth-year');
    const sessionCity = getUser()?.city || '';
    const sessionBirthYear = getUser()?.birth_year || '';
    let prefInterests = Array.isArray(getUser()?.interests) ? getUser().interests : [];
    let prefTempo = getUser()?.social_tempo || 'povremeno';
    let pending = {};
    try {
      const stored = JSON.parse(localStorage.getItem(`mitmi_user_prefs_${getUser()?.id || 'guest'}`) || '{}');
      if ((!prefInterests || !prefInterests.length) && Array.isArray(stored.interests)) prefInterests = stored.interests;
      if ((!prefTempo || prefTempo === 'povremeno') && stored.social_tempo) prefTempo = stored.social_tempo;
    } catch (e) {
      console.warn('[svita] onboarding prefs storage read failed:', e?.message || e);
    }
    try {
      pending = JSON.parse(localStorage.getItem('mitmi_pending_register_profile') || '{}') || {};
    } catch (e) {
      pending = {};
      console.warn('[svita] pending register profile read failed:', e?.message || e);
    }
    if (cityEl && sessionCity && !cityEl.value) cityEl.value = sessionCity;
    if (birthYearEl && sessionBirthYear && !birthYearEl.value) birthYearEl.value = sessionBirthYear;
    if (cityEl && !cityEl.value && pending.city) cityEl.value = pending.city;
    if (birthYearEl && !birthYearEl.value && pending.birth_year) birthYearEl.value = pending.birth_year;
    if ((!prefInterests || !prefInterests.length) && Array.isArray(pending.interests)) prefInterests = pending.interests;
    if ((!prefTempo || prefTempo === 'povremeno') && pending.social_tempo) prefTempo = pending.social_tempo;
    _applyOnboardingPreferenceState(prefInterests, prefTempo);
  }, 0);
}

function prefillRegisterUserBasics() {
  const cityEl = document.getElementById('reg-city');
  const birthYearEl = document.getElementById('reg-birth-year');
  let pending = {};
  try {
    pending = JSON.parse(localStorage.getItem('mitmi_pending_register_profile') || '{}') || {};
  } catch (e) {
    pending = {};
    console.warn('[svita] register prefill storage read failed:', e?.message || e);
  }
  if (cityEl && pending.city && !cityEl.value) cityEl.value = pending.city;
  if (birthYearEl && pending.birth_year && !birthYearEl.value) birthYearEl.value = pending.birth_year;
  _applyRegisterPreferenceState(
    Array.isArray(pending.interests) ? pending.interests : [],
    pending.social_tempo || 'povremeno'
  );
}
...
function validateUserOnboardingStep(step = 1) {
  if (step === 1) {
    ...
    const birthYearRaw = document.getElementById('ob-birth-year')?.value?.trim() || '';
    const birthYear = typeof _normalizeBirthYear === 'function' ? _normalizeBirthYear(birthYearRaw) : null;
    clearFieldError('ob-city');
    clearFieldError('ob-birth-year');
    if (!city) {
    ...
    if (!birthYear) {
      showFieldError('ob-birth-year', 'Unesi ispravno godište.');
      _authToast('Unesi godište', 'error');
      return false;
    }
  }
  if (step === 2) {
...
async function saveOnboarding() {
  ...
  const city = document.getElementById('ob-city')?.value?.trim() || '';
  const birthYear = typeof _normalizeBirthYear === 'function'
    ? _normalizeBirthYear(document.getElementById('ob-birth-year')?.value?.trim() || '')
    : null;
  ...
  try {
    ...
    try {
      ...
      console.warn('[svita] onboarding prefs storage unavailable:', e?.message || e);
    }
    ['city-label','home-city-display','browse-city-label','browse-home-city-label'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.textContent = city;
    });
    if (typeof _normalizeBrowseCityLabel === 'function') {
...
function resetVenueOnboarding() {
  ...
  const typeEl = document.getElementById('vob-type');
  if (typeEl) {
    typeEl.dataset.userTouched = 'false';
  ...
  if (typeof _applyOrganizerTypeSuggestion === 'function') {
    _applyOrganizerTypeSuggestion({
      ...
      typeId: 'vob-type',
      cityId: 'vob-city',
      descriptionId: 'vob-description',
      addressId: 'vob-public-address',
      websiteId: 'vob-website',
      instagramId: 'vob-instagram'
    });
  }
}
...
function validateVenueOnboarding() {
  ...
  const publicAddress = document.getElementById('vob-public-address')?.value?.trim() || '';
  const websiteUrl = document.getElementById('vob-website')?.value?.trim() || '';
  const instagramHandle = typeof _normalizeOrganizerInstagram === 'function'
    ? _normalizeOrganizerInstagram(document.getElementById('vob-instagram')?.value?.trim() || '')
    : (document.getElementById('vob-instagram')?.value?.trim() || '').replace(/^@+/, '');
  const publicContactEmail = document.getElementById('vob-public-email')?.value?.trim() || '';
  const publicContactPhone = document.getElementById('vob-public-phone')?.value?.trim() || '';
  ...
  if (typeof _isValidOptionalUrl === 'function' && !_isValidOptionalUrl(websiteUrl)) {
    return { ok:false, message:'Unesi ispravan website link koji počinje sa http:// ili https://.' };
  }
  if (typeof _isValidOptionalEmail === 'function' && !_isValidOptionalEmail(publicContactEmail)) {
    return { ok:false, message:'Unesi ispravan javni email.' };
  }
...
function validateVenueOnboardingStep(step = 1) {
  const venueName = document.getElementById('vob-name')?.value?.trim() || '';
  const city = document.querySelector('#vob1 [data-venue-city]')?.value?.trim() || '';
  const desc = document.getElementById('vob-description')?.value?.trim() || '';
  const websiteUrl = document.getElementById('vob-website')?.value?.trim() || '';
  const publicContactEmail = document.getElementById('vob-public-email')?.value?.trim() || '';
  if (step === 1) {
    if (!venueName) return { ok:false, message:'Unesi naziv lokala ili organizacije.' };
    if (!city) return { ok:false, message:'Unesi grad u kom se lokal nalazi.' };
    return { ok:true };
  }
  if (step === 2) {
    if (!desc) return { ok:false, message:'Napiši kratak opis lokala ili organizatora.' };
    if (typeof _isValidOptionalUrl === 'function' && !_isValidOptionalUrl(websiteUrl)) {
      return { ok:false, message:'Unesi ispravan website link koji počinje sa http:// ili https://.' };
    }
    if (typeof _isValidOptionalEmail === 'function' && !_isValidOptionalEmail(publicContactEmail)) {
...
function vobNext() {
  if (vobStep === 1) {
    const validation = validateVenueOnboardingStep(1);
    if (!validation.ok) {
  ...
  if (vobStep === 2) {
    const validation = validateVenueOnboardingStep(2);
    if (!validation.ok) {
      _authToast(validation.message, 'error');
      return;
    }
  ...
  try {
    ...
    await _supaFetch('/rest/v1/venues', {
      ...
      body: JSON.stringify({
        ...
        public_address: payload.publicAddress || null,
        website_url: payload.websiteUrl || null,
        instagram_handle: payload.instagramHandle || null,
        public_contact_email: payload.publicContactEmail || null,
        public_contact_phone: payload.publicContactPhone || null,
        status:      'pending'
      })
    });
    ...
    console.error('[svita] venue onboarding:', e);
