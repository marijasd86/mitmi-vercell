...
function _renderChatContext(context = null) {
  ...
  metaEl.textContent = context.inviteTitle
...
function _syncEventPlanUi(plan = null) {
  ...
  if (plan.status === 'confirmed') {
    ...
    if (proposeTxt) proposeTxt.textContent = 'Zajednički odlazak potvrđen';
    return;
  }
...
function _syncChatActionMenu() {
  ...
  if (statusRow) statusRow.style.display = hasEventPlan ? 'flex' : 'none';
  if (proposeBtn) proposeBtn.style.display = hasEventPlan ? 'flex' : 'none';
...
function _renderInboxRows(rows = []) {
  return rows.map(item => {
    ...
    const previewText = item.reviewPending
      ? (item.reviewPreview || 'Oceni iskustvo sa ovog događaja')
      : preview;
  ...
  try {
    ...
    const items = chatRows.map(row => {
      ...
      return {
        ...
        unreadCount: latest?.created_at && (!row.last_read_at || new Date(latest.created_at) > new Date(row.last_read_at)) && latest?.sender_id !== myId ? 1 : 0,
...
async function _findDirectChatWithProfile(profileId, eventId = null) {
  ...
  try {
    const rows = await _supaGet('chat_participants', {
      select: 'chat_id,user_id,hidden_at,chats!inner(id,chat_type,title,created_at,event_id)',
      user_id: `in.(${myId},${profileId})`,
      'chats.chat_type': 'eq.direct',
      'chats.event_id': eventId ? `eq.${eventId}` : 'is.null',
      limit: '50'
    });
...
async function openDirectChat(profileId, name = 'Direktna poruka', context = null) {
  ...
  try {
    ...
    if (eventContext?.eventId) {
      const plan = await _loadEventPairPlan(eventContext.eventId, profileId, eventContext.planId || null);
...
function _setDMStatusInternal(status, options = {}) {
  ...
  if (status === 'potvrdeno') {
    btn.disabled = true;
    btnText.textContent = 'Plan potvrđen';
    ...
    btn.disabled = false;
    btnText.textContent = 'Plan je u dogovoru';
    btn.style.background = 'linear-gradient(135deg, var(--amber2) 0%, var(--amber) 100%)';
    btn.style.color = '#fff';
  } else {
    btn.disabled = false;
    btnText.textContent = 'Označi plan u dogovoru';
...
async function _upsertEventPairPlan() {
  ...
  const refreshed = await _loadEventPairPlan(_activeChatContext.eventId, _activeChatProfileId, _activeChatContext.planId || null);
  _syncEventPlanUi(refreshed);
  return refreshed;
}
