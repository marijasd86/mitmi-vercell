...
async function _ensureChatContextEventTiming(context = null) {
  ...
  try {
    ...
    console.warn('[svita] _ensureChatContextEventTiming:', e.message);
    return normalized;
  }
}
...
async function _isBlockedBetweenMeAnd(profileId) {
  ...
  try {
    ...
    console.warn('[svita] _isBlockedBetweenMeAnd:', e.message);
    return false;
  }
}
...
function _renderChatMessageList(items = []) {
  ...
  const isGroupChat = _activeChatType === 'group';
  let prevSenderId = null;
  items.forEach(m => {
    const div = document.createElement('div');
    const me = m.me || m.sender_id === myId;
    const senderId = m.sender_id || m.profiles?.id || null;
    const grouped = senderId && prevSenderId && senderId === prevSenderId;
    ...
    if (!me) {
      ...
      if (!grouped) {
        ...
        if (sender.avatar_url) {
          av.style.backgroundImage = `url(${sender.avatar_url})`;
    ...
    if (!me && isGroupChat && !grouped) {
      const senderName = String(m.profiles?.display_name || m.profiles?.username || '').trim();
      if (senderName) {
    ...
    const clock = m.time || new Date(m.created_at || Date.now()).toLocaleTimeString('sr', { hour:'2-digit', minute:'2-digit' });
    if (me && m.pending) {
      time.classList.add('is-pending');
      time.textContent = `◷ ${clock}`;
    } else if (me && m.failed) {
      time.classList.add('is-failed');
      time.textContent = `${_langText('Nije poslato', 'Not sent')} · ${clock}`;
    } else if (me) {
      time.textContent = `✓ ${clock}`;
    } else {
      time.textContent = clock;
...
async function openDirectChat(profileId, name = _langText('Direktna poruka', 'Direct message'), context = null) {
  ...
  try {
    ...
    console.error('[svita] openDirectChat:', e);
...
function openChat(type, name, sub, chatId) {
  ...
  if (msgs) {
    ...
    sourceMsgs.forEach(m => {
      ...
      if (grouped) div.classList.add('grouped');
      div.classList.add('msg-enter');
      if (!me && !grouped && (m.av || m.profiles?.id)) {
        const av = document.createElement('div');
        av.className = 'av av-32 ' + (m.color || 'av-purple');
        av.style.cursor = 'pointer';
        if (m.profiles?.avatar_url) {
          av.style.backgroundImage = `url(${m.profiles.avatar_url})`;
          av.style.backgroundSize = 'cover';
          av.style.backgroundPosition = 'center';
          av.textContent = '';
        } else {
  ...
  if (miniName) miniName.textContent = name;
  if (miniAv) {
    const peerMsg = dbMessages.find(m => m.profiles?.id && m.sender_id !== getUser()?.id) || dbMessages.find(m => m.profiles?.id) || null;
    const peerAvatar = peerMsg?.profiles?.avatar_url || '';
    if (peerAvatar) {
      miniAv.style.backgroundImage = `url(${peerAvatar})`;
      miniAv.style.backgroundSize = 'cover';
      miniAv.style.backgroundPosition = 'center';
      miniAv.textContent = '';
    } else {
      miniAv.style.backgroundImage = '';
      miniAv.style.backgroundSize = '';
      miniAv.style.backgroundPosition = '';
      miniAv.textContent = name.charAt(0);
    }
  }
  ...
  nav('chat');
  } catch(e) { console.error('[svita] openChat error:', e); nav('chat'); }
  })();
}
