...
function _renderEventPhotoThumb(src = '', removable = false) {
...
async function loadEventPhotos(eventId = null) {
  ...
  try {
    const rows = await _supaGet('event_photos', {
    ...
    items.forEach(item => {
      ...
      grid.appendChild(_renderEventPhotoThumb(item.photo_url, false));
    });
    checkEvPhotoEmpty();
    return items;
  } catch (e) {
...
async function handleEventPhotos(input) {
  ...
  const grid  = document.getElementById('ev-photo-grid');
  const empty = document.getElementById('ev-photo-empty');
  showToast(`Obrađujem ${input.files.length} sliku/e...`, 'info', 1500);

  for (const file of Array.from(input.files)) {
    const compressed = await compressImage(file, 800, 0.75);
    _eventPhotos.push(compressed);
    if (empty) empty.style.display = 'none';

    if (grid) grid.appendChild(_renderEventPhotoThumb(compressed, true));

    // Upload u pozadini sa stvarnim event_id
    try {
      const userId  = getUser()?.id;
      ...
      await _supaFetch('/rest/v1/event_photos', {
        ...
        body: JSON.stringify({
          ...
          photo_url: url,
          storage_path: path,
          display_order: Math.max(0, (grid?.querySelectorAll('[data-event-photo-thumb="1"]').length || 1) - 1)
        })
      });
    } catch(e) {
  ...
  showToast(`${input.files.length} foto dodato ✓`, 'success', 2000);
  input.value = '';
}
...
function _renderNotificationRows(items = [], opts = {}) {
  ...
  container.innerHTML = _notificationFeed.map((item, index) => `
    <div class="notif-item${item.read ? '' : ' notif-unread'}" onclick="handleNotificationClick(${index},'${_escHtml(item.id || '')}',this)">
...
async function loadNotifications() {
  ...
  try {
    ...
    _renderNotificationRows(notifs.map(n => ({
      id: n.id,
      type: n.type,
      title: n.title,
      body: n.body,
...
function openNotificationItem(itemOrType = '') {
  ...
  if (normalized.includes('review')) {
    nav('profile');
    showToast('Ocene će uskoro biti povezane sa stvarnim događajima', 'info', 1800);
    return;
  }
...
async function renderFallbackNotifications() {
  const [inviteItems, messageItems, followerItems, reminderItems] = await Promise.all([
    _loadInviteNotificationItems(),
    _loadMessageNotificationItems(),
    _loadFollowerNotificationItems(),
    _loadFollowedEventReminderItems()
  ]);
  const items = [...inviteItems, ...messageItems, ...followerItems, ...reminderItems]
...
async function toggleVenueFollow(btn) {
  ...
  if (_currentPublicVenueTarget && _isOrganizerEntity(_currentPublicVenueTarget)) {
    showToast('Ghost organizer još nema follow sistem. Kad organizer preuzme profil, moći ćeš da ga pratiš.', 'info', 2200);
    return;
  }
