...
let _saveVenueInFlight = false;

function _bindEditVenueSaveButtons() {
  const buttons = [
    document.getElementById('edit-venue-save-btn'),
    document.getElementById('edit-venue-save-btn-bottom')
  ].filter(Boolean);
  buttons.forEach((btn) => {
    if (!btn || btn.dataset.boundSaveVenue === '1') return;
    btn.dataset.boundSaveVenue = '1';
...
function _normalizeVenueTarget(entity = null) {
  ...
  if (_isOrganizerEntity(entity)) {
    ...
    const notesCover = (() => {
      const rawNotes = String(entity.source_notes || '');
      const match = rawNotes.match(/\[cover_url:([^\]]+)\]/i);
    ...
    return {
      ...
      cover_url: entity.cover_url || notesCover || '',
...
function _setVenueAvatarBubble(elementId = '', label = '', imageUrl = '') {
  ...
  el.textContent = _venueInitial(label);
...
async function renderPublicVenueProfile(venue = null) {
  ...
  _setVenueAvatarBubble('vp-av', data.venue_name || '', data.avatar_url || data.cover_url || '');
...
function _renderVenueDashboard(venue = null) {
  ...
  if (publicAvatar) {
    _setVenueAvatarBubble('vp-av', venue.venue_name || '', venue.avatar_url || venue.cover_url || '');
  }
