...
async function renderPublicVenueProfile(venue = null) {
  ...
  if (badge) badge.textContent = isOrganizer ? (data.status === 'claimed' ? _langText('Profil povezan', 'Claimed profile') : _langText('Status: u pripremi', 'Status: in setup')) : _venueBadgeText(data.status);
