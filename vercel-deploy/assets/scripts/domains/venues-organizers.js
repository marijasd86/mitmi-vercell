...
function _organizerBadgeText(status = '') {
  ...
  if (status === 'unclaimed') return _langText('Profil još nije preuzet', 'Unclaimed');
...
async function _loadVenuePublicEvents(venue = null) {
  ...
  try {
    ...
    const rows = await _supaGet('events', {
      select: 'id,creator_id,venue_id,organizer_id,title,description,category,event_tags,city,location_name,starts_at,capacity,attendee_count,cover_url,avg_rating,rating_count,is_published,is_cancelled,created_at',
      or: orClause,
