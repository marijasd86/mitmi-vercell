...
function _organizerBadgeText(status = '') {
  ...
  if (status === 'unclaimed') return 'Organizer';
...
function _normalizeVenueTarget(entity = null) {
  ...
  if (_isOrganizerEntity(entity)) {
    const publicType = _organizerTypeLabel(
      entity.organizer_type || entity.venue_type || (entity.status === 'claimed' ? 'Organizer profil' : 'Organizer u pripremi')
    );
    return {
      ...entity,
      id: entity.id,
      ...
      city: entity.city || _langText('Srbija', 'Serbia'),
...
async function renderPublicVenueProfile(venue = null) {
  ...
  setText('vp-followers-label', isOrganizer ? _langText('status', 'status') : _langText('pratilaca', 'followers'));
  setText('vp-followers', isOrganizer ? (data.status === 'claimed' ? _langText('povezan', 'claimed') : _langText('u pripremi', 'in setup')) : String(followersCount));
  ...
  if (badge) badge.textContent = isOrganizer ? (data.status === 'claimed' ? _langText('Profil povezan', 'Claimed profile') : _langText('Profil još nije preuzet', 'Profile not claimed yet')) : _venueBadgeText(data.status);
  ...
  if (secondaryBtn) {
    ...
    if (isOrganizer && data.status === 'unclaimed') {
      ...
      if (isLoggedIn() && ['venue', 'admin'].includes(role)) {
        ...
        secondaryBtn.textContent = claimRequest?.status === 'pending' ? _langText('Zahtev poslat', 'Request sent') : _langText('Zatraži preuzimanje', 'Request claim');
