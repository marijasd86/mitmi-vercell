...
function _renderInboxRows(rows = []) {
  return rows.map(item => {
    ...
    const hasAvatar = !!item.avatarUrl && item.type === 'dm';
    const avatarStyle = hasAvatar
      ? ` style="background-image:url('${_safeCssUrl(item.avatarUrl)}');background-size:cover;background-position:center;color:transparent"`
      : (item.type === 'group' ? ' style="border-radius:10px"' : '');
    return `<div class="chat-row${item.unreadCount > 0 ? ' chat-row-unread' : ''}" onclick="${onclick}"><div class="av av-40 av-purple"${avatarStyle}>${_escHtml((item.title || 'P').charAt(0).toUpperCase())}</div><div class="chat-info"><div class="chat-name-row"><div class="chat-name">${title}</div>${kind}${reviewBadge}${unreadBadge}</div>${subtitleRow}<div class="chat-preview">${previewText}</div></div><div class="chat-meta"><div class="chat-time">${time}</div><button class="btn btn-ghost btn-sm chat-row-delete" onclick="event.stopPropagation();hideConversation('${_escHtml(item.chatId || '')}')" title="Ukloni razgovor">Ukloni</button></div></div>`;
  }).join('');
}
...
function _renderInboxSections(box, items = []) {
  ...
  box.innerHTML = `${grouped.group.length ? `<div class="chat-list-section">GRUPNI CHATOVI</div>${_renderInboxRows(grouped.group)}` : ''}${grouped.eventDm.length ? `<div class="chat-list-section" style="margin-top:8px">DOGOVORI ZA DOGAĐAJE</div>${_renderInboxRows(grouped.eventDm)}` : ''}${grouped.dm.length ? `<div class="chat-list-section" style="margin-top:8px">DIREKTNE PORUKE</div>${_renderInboxRows(grouped.dm)}` : ''}${!grouped.group.length && !grouped.eventDm.length && !grouped.dm.length ? `<div class="draft-empty">${_langText('Još nema poruka.<br>Nađi ekipu ili napravi plan — pa da krene priča.', 'No messages yet.<br>Find your crew or create a plan, then start the conversation.')}</div>` : ''}`;
}
  ...
  try {
    ...
    const items = chatRows.map(row => {
      ...
      return {
        ...
        avatarUrl: isDM ? (peer.avatar_url || '') : '',
...
async function _loadChatReadState(chatId) {
  if (!chatId || !_isSupabaseConfigured()) return {};
  try {
    ...
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      ...
      if (!userId) return;
      state[userId] = row?.last_read_at || null;
    });
    return state;
  } catch (e) {
    console.warn('[svita] _loadChatReadState:', e.message);
    return {};
  }
}
...
async function refreshActiveChatMessages() {
  ...
  if (typeof _loadChatReadState === 'function') {
    try { globalThis._activeChatReadState = await _loadChatReadState(_activeChatId); } catch (e) {}
  }
