function _renderEventDetailUnavailable(message = 'Događaj trenutno nije dostupan.') {
  ...
  const priceWrap = document.getElementById('event-ticket-price-wrap');
  const priceEl = document.getElementById('event-ticket-price');
  ...
  const coverEditBtn = document.getElementById('event-cover-edit-btn');
  const photoManageCopy = document.getElementById('ev-photo-manage-copy');

  if (hero) {
    hero.className = 'ev-hero ev-hero--empty';
  ...
  if (priceEl) priceEl.textContent = 'Cena ulaznice nije upisana';
...
function renderEventDetail(eventCard = null) {
  ...
  if (hero) {
    const heroCover = event.cover_url || _getEventCover(event.id) || '';
    const cleanHeroCover = typeof _coverUrlWithoutFocus === 'function' ? _coverUrlWithoutFocus(heroCover) : heroCover;
    const hasHeroCover = !!cleanHeroCover;
    hero.className = hasHeroCover ? `ev-hero ${event.bg || 'ev-img-a'}` : 'ev-hero ev-hero--empty';
    hero.style.backgroundImage = cleanHeroCover ? `url(${_safeCssUrl(cleanHeroCover)})` : '';
    hero.style.backgroundSize = hasHeroCover ? 'cover' : '';
    hero.style.backgroundPosition = hasHeroCover
      ? (typeof _coverBackgroundPosition === 'function' ? _coverBackgroundPosition(heroCover, 'center') : 'center')
      : '';
  }
  ...
  const dateText = (typeof _formatDayMonth === 'function' ? _formatDayMonth(date, { withWeekday: true }) : '') || _langText('Termin uskoro', 'Time soon');
  const timeText = date.toLocaleTimeString('sr-Latn', { hour:'2-digit', minute:'2-digit' });
  const primaryMetaText = _formatEventDateTimeLine(raw);
  if (dateEl) dateEl.textContent = dateText;
  if (timeEl) timeEl.textContent = timeText;
  if (primaryMetaEl) primaryMetaEl.textContent = primaryMetaText || `${dateText} u ${timeText}`;
  ...
  const ticketPriceLabel = _formatEventTicketPrice(raw);
  if (priceWrap) priceWrap.style.display = ticketPriceLabel ? '' : 'none';
  if (priceEl) priceEl.textContent = ticketPriceLabel;
  ...
  if (capEl) capEl.textContent = capacity > 0 ? `${attendees} / ${capacity}` : _langText('Broj mesta nije ograničen', 'Spots are not limited');
  ...
  if (ownerBtn) {
    ...
    Promise.resolve(_canEditEventFromDetail(event))
      .then(canEdit => {
        ...
        if (coverEditBtn) coverEditBtn.style.display = canEdit ? 'inline-flex' : 'none';
      })
      .catch(() => {});
  }
  if (adminHideBtn) {
  ...
  if (socialBtn) {
    socialBtn.onclick = () => {
      if (!event?.id) {
        showToast(_langText('Događaj trenutno nije dostupan za plan', 'This event is currently unavailable for plans'), 'info', 1800);
        return;
      }
      openCreatePlanForEvent(event.id);
    };
  }
  if (chatBtn) {
    chatBtn.onclick = () => {
      if (!event?.id) {
        showToast(_langText('Event chat trenutno nije dostupan', 'Event chat is currently unavailable'), 'info', 1800);
        return;
      }
      openEventGroupChat(event.id, event.title || raw.title || 'Događaj', raw.creator_id || null);
    };
  }
  if (photoAddBtn) photoAddBtn.style.display = 'none';
  if (coverEditBtn) coverEditBtn.style.display = 'none';
  ...
  if (typeof _canManageEventPhotos === 'function') {
    Promise.resolve(_canManageEventPhotos(event.id))
      .then(canManage => {
        ...
        if (coverEditBtn) coverEditBtn.style.display = canManage ? 'inline-flex' : 'none';
        if (photoManageCopy) {
  ...
  if (event?.id) {
    ...
    const plansBox = document.getElementById('event-plans-list');
    if (plansBox) {
      plansBox.innerHTML = `<div class="draft-empty">${_langText('Planovi trenutno nisu dostupni za ovaj događaj.', 'Plans are currently unavailable for this event.')}</div>`;
    }
  }
...
async function adminHideEventFromDetail(eventCard = null) {
  ...
  try {
    ...
    console.warn('[svita] adminHideEventFromDetail:', e.message);
  ...
  if (_isSupabaseConfigured()) {
    try {
      const rows = await _supaGet('events', {
        select: 'id,creator_id,venue_id,organizer_id,title,description,category,event_tags,city,location_name,public_address,organizer_name_override,starts_at,capacity,attendee_count,ticket_price_rsd,cover_url,avg_rating,rating_count,is_published,is_cancelled,created_at',
        id: `eq.${eventId}`,
        limit: '1'
      });
      const row = Array.isArray(rows) ? rows[0] : null;
      if (row) {
        const fresh = _mapDbEventToCard(row);
        card = {
          ...(card || {}),
          ...fresh,
          cover_url: fresh.cover_url || card?.cover_url || _getEventCover(eventId) || ''
        };
      }
    } catch (e) {
      console.warn('[svita] openEventById:', e.message);
    }
  }
  nav('event');
  if (!card) {
