function _applyProfileToSession(profile = {}) {
  if (!_session) return;
  const resolvedRole = profile.role ?? _session.user?.user_metadata?.role ?? _session.user?.user_role ?? _session.user_role ?? 'user';
  const nextUser = {
    ...(_session.user || {}),
...
function _renderProfileSocials(profile = {}) {
  ...
  if (!signals.length) signals.push(`<div class="soc-chip">${_langText('svita profil', 'svita profile')}</div>`);
...
function _renderMyProfile(profile = null) {
  ...
  const profileCode = username || '@' + _defaultUsername(displayName || email || 'svita');
...
async function copyMyProfileCode() {
  ...
  const code = profile.username
    ...
    : '@' + _defaultUsername(profile.display_name || _profileEmail() || 'svita');
  try {
    ...
    console.warn('[svita] copyMyProfileCode:', e.message);
  }
...
async function loadMyProfile() {
  ...
  try {
    ...
    console.warn('[svita] loadMyProfile:', e.message);
    _renderMyProfile({});
    _renderProfileOrganizerSummary(null);
...
async function saveEditedProfile() {
  ...
  try {
    ...
    console.error('[svita] saveEditedProfile:', e);
  ...
  try {
    ...
    if (_isSupabaseConfigured() && isLoggedIn() && typeof _uploadProfileAvatarDataUrl === 'function') {
      try {
        ...
        console.warn('[svita] handleProfileAvatar upload:', uploadErr.message);
      }
    }
    const previewEl = document.getElementById('edit-profile-avatar-preview');
    const removeBtn = document.getElementById('edit-profile-avatar-remove');
