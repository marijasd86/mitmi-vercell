...
function _renderProfileBasicsSummary(profile = null) {
  ...
  const chips = [];
  if (birthYear) chips.push(`🎂 ${birthYear}`);
  if (socialTempo) chips.push(`🫶 ${_socialTempoLabel(socialTempo)}`);
  interests.slice(0, 3).forEach((item) => chips.push(`✨ ${_interestLabel(item)}`));
  shell.style.display = chips.length ? 'flex' : 'none';
  shell.innerHTML = chips
...
function _renderProfileSocials(profile = {}) {
  ...
  if (profile.avatar_url) signals.push(`<div class="soc-chip">${_langText('🖼 Ima sliku', '🖼 Has photo')}</div>`);
  if (profile.city) signals.push(`<div class="soc-chip">📍 ${_escHtml(profile.city)}</div>`);
  if (eventsCount > 0) signals.push(`<div class="soc-chip">${_langText(`🎟 ${eventsCount} događaja`, `🎟 ${eventsCount} events`)}</div>`);
  if (ratingCount > 0) signals.push(`<div class="soc-chip">${_langText(`★ ${ratingCount} ocena`, `★ ${ratingCount} reviews`)}</div>`);
  if (!signals.length) signals.push(`<div class="soc-chip">${_langText('🌱 Nov profil', '🌱 New profile')}</div>`);
...
async function saveEditedProfile() {
  ...
  if (!birthYear) {
    showToast(_langText('Unesi godište', 'Enter your birth year'), 'error');
    return;
  }
  if (interests.length < 3) {
    showToast(_langText('Izaberi bar 3 interesovanja', 'Choose at least 3 interests'), 'error');
    return;
  }
