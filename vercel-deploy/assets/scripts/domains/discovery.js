...
function ttAction(event, action) {
  ...
  if (action === 'like') {
    ...
    showToast((current.swipeType || 'event') === 'invite' ? 'Sačuvan je događaj uz ovaj poziv' : 'Ulaziš u ovaj događaj', 'success', 1400);
  ...
  if (action === 'chat') {
    ...
    if ((current.swipeType || 'event') === 'invite' && current.creatorId && current.inviteId && current.eventId) {
      return openInviteDirectChat(
        current.creatorId,
        current.creatorName || 'Direktna poruka',
        current.inviteId,
        current.eventId,
        current.title || 'Poziv',
        current.eventTitle || 'Događaj',
        current.planId || ''
      );
    }
    if ((current.swipeType || 'event') === 'invite' && current.creatorId && !current.eventId) {
      return openDirectChat(current.creatorId, current.creatorName || 'Direktna poruka');
    }
...
function ttLoadCard(data) {
  ...
  set('tt-cat',        `${typeLabel} · ${emoji} ${(data.cat ? data.cat.charAt(0).toUpperCase() + data.cat.slice(1) : '')}`.trim());
  ...
  set('tt-like-count', (data.swipeType || 'event') === 'invite' ? 'Sačuvaj događaj' : 'Ulazim');
  const followBtn = document.getElementById('tt-follow-btn');
  if (followBtn) {
  ...
  if (backCat) {
    ...
    backCat.textContent = `${nextTypeLabel} · ${(TT_CAT_EMOJI[nextData?.cat || data.cat] || '📅')} ${((nextData?.cat || data.cat || '').replace(/^./, c => c.toUpperCase()))}`;
