...
function ttSwipe(dir) {
  ...
  setTimeout(() => {
    ...
    swipeIdx = (swipeIdx + 1) % Math.max(1, swipeDataNow.length);
...
function _initTikTokSwipeBindings() {
  ...
  ttFront.addEventListener('touchmove', e => {
    ...
    const travel = vertical ? absY : absX;
    if (travel < 4) return;

    // Prevent scroll hijacking while dragging the card.
    e.preventDefault();

    const lift = Math.min(1, travel / 140);
    if (vertical) {
      ttFront.style.transform = `translateY(${dy}px) scale(${1 - lift * 0.03})`;
    } else {
      ttFront.style.transform = `translateX(${dx}px) rotate(${dx * 0.05}deg) scale(${1 - lift * 0.02})`;
    }
    const likeInd = document.getElementById('tt-like-ind');
    const skipInd = document.getElementById('tt-skip-ind');
    if (vertical) {
      ...
      if (likeInd) likeInd.style.opacity = String(Math.min(1, dx / 90));
      if (skipInd) skipInd.style.opacity = String(Math.min(1, -dx / 90));
    }
  }, { passive: false });
  ...
  ttFront.addEventListener('touchend', e => {
    ...
    const threshold = 70;
    if (vertical) {
      if (dy < -threshold) ttSwipe('up');
      else if (dy > threshold) ttSwipe('down');
      return;
    }
    if (dx > threshold) ttSwipe('right');
    else if (dx < -threshold) ttSwipe('left');
  }, { passive: true });
