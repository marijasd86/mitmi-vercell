...
function _swipeTypeLabel(kind = 'event') {
  return _swipeKind(kind) === 'plan' ? 'Plan' : 'Događaj';
}

function _swipeMetaCopy(data = {}) {
  const kind = _swipeKind(data.swipeType || 'event');
  const venue = data.venue || data.location_name || 'Lokacija uskoro';
  if (kind === 'plan') {
    return {
      meta: `💬 ${venue}`,
      going: 'Ako ti deluje zanimljivo, javi se i otvori chat'
    };
  }
  return {
    meta: `📍 ${venue}`,
    going: data.going || 'Sačuvaj događaj pa odluči kasnije'
  };
}
...
function setSwipeFilter(el, cat) {
  ...
  if (_swipeFiltered && _swipeFiltered.length === 0) {
    showToast('Nema događaja ili planova u ovoj kategoriji', 'info', 2000);
    _swipeFiltered = null;
    return;
  }
  swipeIdx = 0;
...
function switchBrowseTab(tab, preserveInput = false) {
  ...
  if (si) {
    if (tab === 'home') {
      ...
      si.placeholder = 'Pretraži planove...';
      si.oninput = function() { doPlanSearch(this.value); };
    } else {
  ...
  try {
    ...
    const organizers = Array.isArray(organizerRows) ? organizerRows
      ...
      .map(organizer => ({
        ...
        card_type: organizer.organizer_type || (organizer.status === 'claimed' ? 'Organizer profil' : 'Organizer u pripremi'),
        followers_count: 0,
