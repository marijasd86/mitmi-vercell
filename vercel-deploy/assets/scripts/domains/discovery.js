...
function setSwipeFilter(el, cat) {
  ...
  if (cat === 'all') {
    ...
    _swipeFiltered = _combinedEventCards().filter(e => typeof _isTonightEvent === 'function' ? _isTonightEvent(e.starts_at || e.date || '') : false);
  } else {
    _swipeFiltered = _combinedEventCards().filter(e => e.cat === cat);
  }
