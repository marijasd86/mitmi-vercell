...
function switchBrowseTab(tab, preserveInput = false) {
  ...
  if (si) {
    if (tab === 'home') {
      si.placeholder = _langText('Pretraži događaje ili organizatore...', 'Search events or organizers...');
      si.oninput = function() {
        const q = this.value;
      ...
      si.placeholder = _langText('Pretraži događaje...', 'Search events...');
      si.oninput = function() { doSearch(this.value); };
    } else if (tab === 'venues') {
      si.placeholder = _langText('Pretraži organizatore...', 'Search organizers...');
      si.oninput = function() { doVenueSearch(this.value); };
    } else if (tab === 'plans') {
      si.placeholder = _langText('Pretraži planove...', 'Search plans...');
