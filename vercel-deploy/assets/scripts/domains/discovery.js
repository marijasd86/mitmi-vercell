...
const TT_BG_MAP = {
  ...
  scena_humor:'ev-img-b', standup:'ev-img-b',
  pozoriste:'ev-img-b',
...
function ttLoadCard(data) {
  ...
  if (bg) {
    ...
    const cleanCoverUrl = typeof _coverUrlWithoutFocus === 'function' ? _coverUrlWithoutFocus(data.cover_url) : data.cover_url;
    bg.style.backgroundImage = cleanCoverUrl ? `url(${cleanCoverUrl})` : '';
    bg.style.backgroundSize = data.cover_url ? 'cover' : '';
    bg.style.backgroundPosition = data.cover_url
      ? (typeof _coverBackgroundPosition === 'function' ? _coverBackgroundPosition(data.cover_url, '50% 82%') : 'center 82%')
      : '';
    bg.style.transform = '';
  }
  const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
  ...
  _setBackCardPreview(data, 'up');
}

function _setBackCardPreview(fallbackData = null, dir = 'up') {
  const swipeDataNow = _getSwipeData();
  const total = swipeDataNow.length;
  let previewData = fallbackData;
  if (total > 1) {
    if (dir === 'down') previewData = swipeDataNow[(swipeIdx - 1 + total) % total];
    else previewData = swipeDataNow[(swipeIdx + 1) % total];
  }
  const backBg = document.getElementById('tt-back-bg');
  if (backBg && previewData) {
    backBg.className = 'tt-bg ' + (TT_BG_MAP[previewData.cat] || 'ev-img-b');
    const cleanCoverUrl = typeof _coverUrlWithoutFocus === 'function' ? _coverUrlWithoutFocus(previewData.cover_url) : previewData.cover_url;
    backBg.style.backgroundImage = cleanCoverUrl ? `url(${cleanCoverUrl})` : '';
    backBg.style.backgroundSize = previewData.cover_url ? 'cover' : '';
    backBg.style.backgroundPosition = previewData.cover_url
      ? (typeof _coverBackgroundPosition === 'function' ? _coverBackgroundPosition(previewData.cover_url, '50% 82%') : 'center 82%')
      : '';
    backBg.style.transform = '';
  }
  const backTitle = document.getElementById('tt-back-title');
  const backCat = document.getElementById('tt-back-cat');
  if (backTitle) backTitle.textContent = previewData?.title || fallbackData?.title || '';
  if (backCat) {
    const typeKind = _swipeKind(previewData?.swipeType || fallbackData?.swipeType || 'event');
    const typeLabel = _swipeTypeLabel(typeKind);
    const categoryName = previewData?.category_label || fallbackData?.category_label || ((previewData?.cat || fallbackData?.cat || '').replace(/^./, c => c.toUpperCase()));
    backCat.textContent = `${typeLabel} · ${(TT_CAT_EMOJI[previewData?.cat || fallbackData?.cat] || '📅')} ${categoryName}`;
    backCat.dataset.kind = typeKind;
  }
}

function ttSwipe(dir, opts = {}) {
  if (ttSwipeAnimating) return;
  const swipeDataNow = _getSwipeData();
  if (!swipeDataNow.length) return;
  const front = document.getElementById('tt-front');
  const back  = document.getElementById('tt-back');
  if (!front) return;
  ttSwipeAnimating = true;
  ...
  const current = _currentSwipeEvent();
  _setBackCardPreview(current, dir === 'down' ? 'down' : 'up');
  ...
  if (back) {
    // Swipe up -> next card enters from bottom (+)
    // Swipe down -> previous card enters from top (-)
    const startY = dir === 'up' ? '60px' : '-60px';
    back.style.transition = 'none';
    back.style.transform = `translateY(${startY})`;
    back.style.opacity = '.7';
    // Force reflow so browser commits start position before transition.
...
function _initTikTokSwipeBindings() {
  ...
  ttFront.addEventListener('touchmove', e => {
    if (!ttTouching || ttSwipeAnimating) return;
    ...
    if (Math.abs(dy) > Math.abs(ttTouchPeakDy)) ttTouchPeakDy = dy;
    const travel = absY;
    if (travel < 4) return;
    ...
    const back = document.getElementById('tt-back');
    if (back) {
      const previewDir = dy >= 0 ? 'down' : 'up';
      _setBackCardPreview(_currentSwipeEvent(), previewDir);
      const backLift = Math.min(1, Math.abs(dy) / 120);
      const baseY = dy >= 0 ? -34 : 34;
      const backY = baseY * (1 - backLift);
  ...
  ttFront.addEventListener('touchend', e => {
    ...
    const resolvedDy = Math.abs(ttTouchPeakDy) > Math.abs(dy) ? ttTouchPeakDy : dy;
    const absX = Math.abs(dx);
    const absY = Math.abs(resolvedDy);
    const elapsed = Math.max(1, Date.now() - ttTouchStartAt);
    const vy = resolvedDy / elapsed;
    const likeInd = document.getElementById('tt-like-ind');
    const skipInd = document.getElementById('tt-skip-ind');
    const threshold = 26;
    ...
    const swipeDir = (resolvedDy < -threshold || vy < -flickVelocity)
      ? 'up'
      : ((resolvedDy > threshold || vy > flickVelocity) ? 'down' : '');
  ...
  // Mouse drag za desktop (vertical, TikTok-like)
  let mouseStartX = 0, mouseStartY = 0, mouseDragging = false;
  let mousePeakDy = 0;
  ttFront.addEventListener('mousedown', e => {
    if (ttSwipeAnimating) return;
  ...
  ttFront.addEventListener('mousemove', e => {
    if (!mouseDragging || ttSwipeAnimating) return;
    const dx = e.clientX - mouseStartX;
    const dy = e.clientY - mouseStartY;
    if (Math.abs(dy) > Math.abs(mousePeakDy)) mousePeakDy = dy;
  ...
  document.addEventListener('mouseup', e => {
    ...
    const dx = e.clientX - mouseStartX;
    const dy = e.clientY - mouseStartY;
    const resolvedDy = Math.abs(mousePeakDy) > Math.abs(dy) ? mousePeakDy : dy;
    ttFront.style.transform = '';
    const back = document.getElementById('tt-back');
    if (back) {
      back.style.transform = '';
    ...
    if (likeInd) likeInd.style.opacity = '0';
    if (skipInd) skipInd.style.opacity = '0';
    if (Math.abs(dx) > Math.abs(resolvedDy)) return;
    if (Math.abs(resolvedDy) > 26) ttSwipe(resolvedDy < 0 ? 'up' : 'down');
  });
}
...
function setSwipeFilter(el, cat) {
  // Azuriraj pills
  const pillsRoot = el?.closest?.('.tt-filters, .pills');
  if (!pillsRoot) return;
  pillsRoot.querySelectorAll('.pill').forEach(p => p.classList.remove('active'));
  ...
  if (cat === 'all') {
    ...
    _swipeFiltered = combined.filter((e) => {
      const tags = Array.isArray(e.tags) ? e.tags : (Array.isArray(e.raw?.event_tags) ? e.raw.event_tags : []);
      if (typeof _matchesEventCategoryFilter === 'function') {
        return _matchesEventCategoryFilter(resolvedCat, e.raw_category || e.cat || '', e.cat || '', tags);
      }
...
async function loadBrowseVenues() {
  ...
  try {
      ...
      if (organizerIds.length) {
        try {
          const eventRows = await _supaGet('events', {
            select: 'organizer_id,starts_at,cover_url',
          ...
          (Array.isArray(eventRows) ? eventRows : []).forEach((row) => {
            ...
            if (!id) return;
            const current = byOrganizer.get(id) || { total: 0, upcoming: 0, cover_url: '' };
            current.total += 1;
            const startsAt = row?.starts_at ? new Date(row.starts_at) : null;
            if (startsAt && startsAt >= today) current.upcoming += 1;
            if (!current.cover_url && row?.cover_url) current.cover_url = row.cover_url;
            byOrganizer.set(id, current);
          });
          organizers.forEach((org) => {
            const stats = byOrganizer.get(org.id) || { total: 0, upcoming: 0, cover_url: '' };
            org.public_events_count = Number(stats.total || 0);
            org.upcoming_events_count = Number(stats.upcoming || 0);
            if (!org.cover_url && stats.cover_url) org.cover_url = stats.cover_url;
          });
        } catch (e) {
    ...
    list.innerHTML = combined.map(venue => {
      ...
      const coverStyle = venue.cover_url ? ` style="${typeof _coverInlineStyle === 'function' ? _coverInlineStyle(venue.cover_url, 'center') : `background-image:url('${_safeCssUrl(venue.cover_url)}');background-size:cover;background-position:center`}"` : '';
