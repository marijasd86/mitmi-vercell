async function syncPendingReviewTasks() {
  ...
  try {
    ...
    console.warn('[svita] syncPendingReviewTasks:', e.message);
    return 0;
  }
}
...
async function loadPendingReviewTasks(options = {}) {
  ...
  try {
    ...
    console.warn('[svita] loadPendingReviewTasks:', e.message);
    if (render) {
      _renderProfileReviewCard([]);
      renderReviewPage([]);
    }
...
async function submitActiveReviewTask() {
  ...
  try {
    ...
    console.warn('[svita] submitActiveReviewTask:', e.message);
