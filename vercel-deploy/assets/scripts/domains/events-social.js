...
async function _loadPlans(params = {}) {
  ...
  try {
    ...
    console.warn('[svita] _loadPlans:', e.message);
    return [];
  }
}
...
function _planCreatorLabel(profile = {}) {
  return profile.display_name || profile.username || _langText('svita korisnik', 'svita user');
}
...
async function loadEventPlans(eventId) {
  ...
  try {
    ...
    console.warn('[svita] loadEventPlans:', e.message);
...
async function loadBrowsePlans() {
  ...
  try {
    ...
    if (!items.length) {
      box.innerHTML = `<div class="draft-empty">${_langText('Još niko nije pokrenuo plan.<br>Baci predlog i vidi ko je raspoložen.', 'No one has started a plan yet.<br>Post one and see who is up for it.')}</div>`;
      return [];
    }
    box.innerHTML = items.map(_renderBrowsePlanCard).join('');
    return items;
  } catch (e) {
    console.warn('[svita] loadBrowsePlans:', e.message);
    globalThis.BROWSE_PLAN_DATA = [];
...
async function loadMyPlans() {
  ...
  try {
    ...
    console.warn('[svita] loadMyPlans:', e.message);
    box.innerHTML = `<div class="draft-empty">${_langText('Planovi trenutno nisu dostupni.', 'Plans are currently unavailable.')}</div>`;
    return [];
  }
}
