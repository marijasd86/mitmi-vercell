...
function _planCreatorLabel(profile = {}) {
  return profile.display_name || profile.username || 'mitmi korisnik';
}
...
function _planActionButtons(item = {}, options = {}) {
  ...
  if (isOwn) return `<span class="tag tag-outline">Tvoj plan</span>`;

  const safeProfileId = _escHtml(profile.id || '');
  const safeName = options.jsName || _escJsArg(name);
  const safeTitle = options.jsTitle || _escJsArg(item.title || 'Plan');
  const safeEventTitle = options.jsEventTitle || _escJsArg(options.eventTitle || item.events?.title || 'Događaj');
  ...
  if (safeEventId) {
    return `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openPlanDirectChat('${safeProfileId}','${safeName}','','${safeEventId}','${safeTitle}','${safeEventTitle}','${safePlanId}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToPlan('','${safeProfileId}','${safeName}','${safeTitle}','${safeEventId}','${safeEventTitle}','${safePlanId}','plan')">Javi se</button></div>`;
  }

  return `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openDirectChat('${safeProfileId}','${safeHtmlName}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();openDirectChat('${safeProfileId}','${safeHtmlName}')">Javi se</button></div>`;
}

function _renderEventPlanRow(item = {}, eventTitle = 'Događaj') {
  const profile = item.profiles || {};
  const name = _planCreatorLabel(profile);
  ...
  const buttons = _planActionButtons(item, {
    ...
    jsTitle: _escJsArg(item.title || 'Plan'),
...
function _renderBrowsePlanCard(item = {}) {
  ...
  const buttons = _planActionButtons(item, {
    ...
    jsName: _escJsArg(name),
    jsTitle: _escJsArg(item.title || 'Plan'),
    jsEventTitle: _escJsArg(event.title || 'Događaj')
  });
...
function _renderMyPlanRow(item = {}) {
  ...
  const label = item.event_id ? 'Otvoreno' : (item.organizer_id ? 'Za mesto' : 'Samostalno');
...
async function loadEventPlans(eventId) {
  ...
  try {
    ...
    const eventTitle = document.getElementById('event-title')?.textContent || items[0]?.events?.title || 'Događaj';
