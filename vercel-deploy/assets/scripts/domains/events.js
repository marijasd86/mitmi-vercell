...
async function applyToPlan(inviteId, creatorId, creatorName = 'mitmi korisnik', inviteTitle = 'Plan', eventId = '', eventTitle = 'Događaj', planId = '', sourceType = 'plan') {
  if (!isLoggedIn()) {
...
async function loadMyCreatedEvents() {
  ...
  try {
    const params = {
      select: 'id,creator_id,venue_id,organizer_id,title,description,category,event_tags,city,location_name,organizer_name_override,starts_at,capacity,attendee_count,cover_url,is_published,is_cancelled,created_at',
