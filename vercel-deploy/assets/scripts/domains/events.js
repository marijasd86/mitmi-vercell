...
function resetCreateForm() {
  ...
  if (locationLabel) locationLabel.textContent = 'Lokacija (okvirna)';
...
function loadCreateForm() {
  ...
  if (!_editingEventId) {
    if (_createFlowMode === 'suggest') {
      ...
      if (reviewNoteCopy) reviewNoteCopy.textContent = 'Pošalji osnovne podatke, a mitmi admin će proveriti detalje i objaviti događaj ako je sve u redu.';
      if (titleLabel) titleLabel.textContent = 'Naziv događaja';
      if (categoryLabel) categoryLabel.textContent = 'Kategorija';
      if (dateLabelEl) dateLabelEl.textContent = 'Datum';
      if (timeLabelEl) timeLabelEl.textContent = 'Vreme';
      if (locationLabel) locationLabel.textContent = 'Lokacija / mesto';
    ...
    if (headerEl) headerEl.textContent = isManagedCreate ? 'Novi event' : 'Tražim društvo';
    if (saveBtn) saveBtn.textContent = isManagedCreate ? 'Objavi event' : 'Objavi plan';
    if (contextEl) contextEl.style.display = 'none';
    if (categoryWrap) categoryWrap.style.display = '';
      if (dateWrap) dateWrap.style.display = '';
    if (timeWrap) timeWrap.style.display = '';
...
function openCreateEntryMenu() {
  ...
  const items = [
    {
      ...
      desc: isEn ? 'Create a simple plan people can join.' : 'Napravi plan i javi da tražiš ekipu.',
      meta: isEn ? 'Now' : 'Odmah',
      metaClass: 'primary',
  ...
  if (caps.canPublishManagedEvents) {
    items.push({
      icon: '◻',
      title: isEn ? 'New event' : 'Novi event',
      desc: isEn ? 'Publish an official event as an organizer.' : 'Dodaj zvanični događaj kao organizator.',
  ...
  items.push({
    ...
    desc: isEn ? 'Send a missing event to admin review.' : 'Pošalji događaj kog još nema u aplikaciji.',
...
function syncAdminUI() {
  ...
  if (createLabel && typeof getRoleCapabilities === 'function') {
    createLabel.textContent = getRoleCapabilities().canPublishManagedEvents ? 'Event' : 'Društvo';
  }
