...
function renderBrowseEventsGrid() {
  ...
  grid.innerHTML = items.map(event => {
    ...
    return `<div class="sq-card" data-cat="${_escHtml(event.raw_category || event.cat)}" data-bucket="${_escHtml(event.cat)}" data-day="${_escHtml(_eventDayBucket(event.date || event.starts_at || ''))}" data-date="${_escHtml(event.date || '')}" onclick="openEventById('${_escHtml(event.id)}')">
