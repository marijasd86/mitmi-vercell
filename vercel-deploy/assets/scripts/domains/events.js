...
function _planMeta(item = {}) {
  ...
  const timeLabel = event.starts_at
    ...
    : (item.starts_at ? _formatEventMeta({ starts_at: item.starts_at, city: item.city, location_name: item.location_name }) : 'Vreme nije upisano');
  return [place, timeLabel].filter(Boolean).join(' · ');
}

function _mapPlanToInviteLike(item = {}) {
  const event = item.events || {};
...
async function loadEventInvites(eventId) {
  ...
  try {
    ...
    const legacyItems = (Array.isArray(rows) ? rows : []).filter(item => !isProfileBlocked(item.creator_id));
    ...
    if (!items.length) items = legacyItems;
    if (!items.length) {
      box.innerHTML = '<div class="draft-empty">Još nema javnih poziva za ovaj događaj.</div>';
      return [];
    }
    ...
    box.innerHTML = items.map(item => {
      ...
      const actionInviteId = linkedInvite?.id || '';
      ...
      return `<div class="inv-row">
        ...
        ${isOwn
          ...
          : actionInviteId
            ? `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openInviteDirectChat('${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}','${_escHtml(actionInviteId)}','${_escHtml(item.event_id || '')}','${_escHtml(item.title || 'Plan').replace(/'/g, '&#39;')}','${_escHtml(eventTitle).replace(/'/g, '&#39;')}','${_escHtml(item.id || '')}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToInvite('${_escHtml(actionInviteId)}','${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}','${_escHtml(item.title || 'Plan')}','${_escHtml(item.event_id || '')}','${_escHtml(eventTitle).replace(/'/g, '&#39;')}')">Prijavi se</button></div>`
            : `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openDirectChat('${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();openDirectChat('${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}')">Javi se</button></div>`}
      </div>`;
    }).join('');
    return items;
  } catch (e) {
  ...
  try {
    ...
    box.innerHTML = items.map(item => {
      ...
      const actionButtons = isOwn
        ...
        : eventId
          ? `<button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openInviteDirectChat('${_escHtml(profile.id || '')}','${safeName}','${_escHtml(item.id)}','${_escHtml(eventId)}','${safeTitle}','${safeEventTitle}','${_escHtml(item.id || '')}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToInvite('${_escHtml(item.id)}','${_escHtml(profile.id || '')}','${safeName}','${safeTitle}','${_escHtml(eventId)}','${safeEventTitle}')">Prijavi se</button>`
...
function openInviteDirectChat(profileId, name = 'Direktna poruka', inviteId = '', eventId = '', inviteTitle = 'Poziv', eventTitle = 'Događaj', planId = '') {
...
async function applyToInvite(inviteId, creatorId, creatorName = 'mitmi korisnik', inviteTitle = 'Poziv', eventId = '', eventTitle = 'Događaj') {
  if (!isLoggedIn()) {
  ...
  if (isProfileBlocked(creatorId)) {
    showToast('Ovaj profil više nije dostupan za prijavu na poziv', 'info', 1800);
    return;
  }
  ...
  try {
    ...
    await openInviteDirectChat(creatorId, creatorName, inviteId, eventId, inviteTitle, eventTitle);
  } catch (e) {
...
function _eventVisualCategory(category = '') {
  ...
  if (['muzika', 'svirka', 'dj', 'standup', 'festival'].includes(normalized)) return 'muzika';
  if (['sport'].includes(normalized)) return 'sport';
  if (['kultura', 'pozoriste', 'izlozba', 'film', 'radionica'].includes(normalized)) return 'kultura';
  if (['kafa', 'bar', 'izlasci'].includes(normalized)) return 'kafa';
  if (['priroda'].includes(normalized)) return 'priroda';
  return 'drugo';
}
...
function _eventSpotsLabel(value) {
  if (value == null || value === '') return 'Bez limita';
  const raw = String(value).trim();
  if (!raw) return 'Bez limita';
  if (/^\d+$/.test(raw)) {
    return Number(raw) > 0 ? `${raw} mesta` : 'Bez limita';
  }
  return raw.toLowerCase() === 'novo' ? 'Bez limita' : raw;
}
...
function getSwipeInviteCards() {
  return (Array.isArray(BROWSE_INVITE_DATA) ? BROWSE_INVITE_DATA : []).map(item => {
    const event = item.events || {};
    const profile = item.profiles || {};
    const name = profile.display_name || profile.username || 'mitmi korisnik';
    ...
    return {
      ...
      swipe_key: `invite-${item.id || eventId || ''}`,
      swipeType: 'invite',
      ...
      title: item.title || 'Plan',
      meta: _planMeta(item),
      date: event.starts_at ? _formatEventMeta(event) : (item.starts_at ? _formatEventMeta({ starts_at: item.starts_at, city: item.city, location_name: item.location_name }) : 'Vreme nije upisano'),
      starts_at: item.starts_at || event.starts_at || '',
      cat,
      bg: _eventBg(cat),
...
async function loadMyCreatedEvents() {
  ...
  try {
    ...
    if (typeof _getMyClaimedOrganizer === 'function') {
      try {
        organizerId = (await _getMyClaimedOrganizer())?.id || null;
      } catch (e) {}
    }
    const params = {
    ...
    if (organizerId) {
      params.or = `(creator_id.eq.${getUser()?.id},organizer_id.eq.${organizerId})`;
    } else {
      params.creator_id = `eq.${getUser()?.id}`;
    }
    const rows = await _supaGet('events', params);
    const mapped = Array.isArray(rows) ? rows.map(_mapDbEventToCard) : [];
...
function renderMyCreatedEvents(items = []) {
  ...
  box.innerHTML = items.map(item => {
    const itemDate = _parseEventDateLocal(item.date || item.starts_at || '') || new Date();
    const label = itemDate < new Date(new Date().setHours(0,0,0,0))
      ? 'Bilo'
      : 'Vreme nije upisano';
...
async function loadMyInvites() {
  ...
  try {
    ...
    const legacyInviteIds = plans.filter(item => item.event_id && item.events).map(item => item.id).filter(Boolean);
    let counts = new Map();
...
function loadCreateForm() {
  ...
  if (cityEl) cityEl.value = raw.city || getUser()?.city || 'Novi Sad';
...
function openCreateEntryMenu() {
  ...
  const items = [
    {
      icon: '◎',
      title: isEn ? 'Find company' : 'Tražim društvo',
      desc: isEn ? 'Post that you are looking for company for an outing or event.' : 'Objavi da tražiš društvo za izlazak ili događaj.',
      meta: isEn ? 'Now' : 'Odmah',
      metaClass: 'primary',
  ...
  if (caps.canPublishManagedEvents) {
    items.push({
      icon: '◻',
      title: isEn ? 'Publish event' : 'Objavi događaj',
      desc: isEn ? 'For organizers and admins: publish an official event to the catalog.' : 'Za organizatore i admin: objavi zvanični događaj u katalogu.',
      meta: isEn ? 'Organizer' : 'Organizer',
      metaClass: 'primary',
  ...
  items.push({
    icon: '↗',
    title: isEn ? 'Suggest an event' : 'Predloži događaj',
    desc: isEn ? 'Send a missing event to admin review. It will not be published immediately.' : 'Pošalji događaj kog još nema u aplikaciji. Ne objavljuje se odmah, već ide adminu na pregled.',
    meta: isEn ? 'Review' : 'Pregled',
    metaClass: 'muted',
...
function handleCreateSubmit() {
  if (_createFlowMode === 'suggest' && !_inviteEventId && !_editingEventId) {
    handleSuggestEventSubmit();
    return;
  }
  handleCreateInvite();
}
...
function renderEventDetail(eventCard = null) {
  ...
  if (socialBtn) {
    socialBtn.onclick = () => openCreateInviteForEvent(event.id);
  }
  if (chatBtn) {
...
let ADMIN_INVITE_LEADS = [];
let _moderationFilter = 'all';

function _normalizeInviteLeadText(value = '') {
  return String(value || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
function _looksLikeStandaloneSocialPlan(item = {}) {
  const title = _normalizeInviteLeadText(item.title || '');
  const description = _normalizeInviteLeadText(item.description || '');
...
function _looksLikeCatalogEventLead(item = {}) {
  const title = _normalizeInviteLeadText(item.eventTitle || item.title || '');
  const location = _normalizeInviteLeadText(item.locationName || item.city || '');
...
function _persistAdminDraftState() {
  _saveStoredList(ADMIN_ORGANIZERS_KEY, ADMIN_ORGANIZERS);
  _saveStoredList(EVENT_DRAFTS_KEY, EVENT_DRAFTS);
}

function _hydrateAdminDraftState() {
  ADMIN_ORGANIZERS = _loadStoredList(ADMIN_ORGANIZERS_KEY, ADMIN_ORGANIZERS);
  EVENT_DRAFTS = _loadStoredList(EVENT_DRAFTS_KEY, EVENT_DRAFTS);
}
...
async function loadAdminInviteLeadsFromBackend(opts = {}) {
  if (!_isSupabaseConfigured()) return ADMIN_INVITE_LEADS;
  try {
    ...
    ADMIN_INVITE_LEADS = rows.map(item => {
      const event = item.events || {};
      const profile = item.profiles || {};
    ...
    if (!opts.silent) renderAdminInviteLeads();
  } catch (e) {
    console.warn('[mitmi] loadAdminInviteLeadsFromBackend:', e.message);
  }
  return ADMIN_INVITE_LEADS;
}
...
function renderLandingHeroEvents() {
  ...
  box.innerHTML = upcoming.map((ev, i) => {
    ...
    const coverStyle = ev.cover_url
      ? `background-image:url('${_escHtml(ev.cover_url)}');background-size:cover;background-position:center`
      : '';
    return `<div class="hero-ev-card" style="animation:cardReveal .3s ease ${i * 0.06}s both;opacity:0" onclick="openEventById('${_escHtml(ev.id || '')}')">
      <div class="hero-ev-img ${_escHtml(ev.bg)}"${coverStyle ? ` style="${coverStyle}"` : ''}>
        <span class="hero-cat-tag"><span class="hero-tag-icon" aria-hidden="true">${emoji}</span>${_escHtml(_catLabel(ev.cat))}</span>
      </div>
      <div class="hero-ev-body">
...
function syncAdminUI() {
  ...
  const inviteLeadCount = ADMIN_INVITE_LEADS.length;
  ...
  const inviteLeadStat = document.getElementById('admin-stat-invite-leads');
  if (inviteLeadStat) inviteLeadStat.textContent = String(inviteLeadCount);
  const inviteLeadBadge = document.getElementById('admin-invite-leads-badge');
  if (inviteLeadBadge) inviteLeadBadge.textContent = `${inviteLeadCount} signala`;
}
...
async function createDraftFromInviteLead(inviteId) {
  const lead = ADMIN_INVITE_LEADS.find(item => item.id === inviteId);
  ...
  try {
    await _supaFetch('/rest/v1/event_drafts', {
      ...
      body: JSON.stringify({
        ...
        source_label: 'invite_signal',
        source_url: null,
        organizer_id: lead.organizerId || null,
        ai_summary: 'Signal je izveden iz korisničkog poziva koji liči na pravi događaj.',
        submitted_by: lead.creatorId || null
      })
    });
    showToast('Signal iz poziva je poslat u draftove', 'success', 2000);
    await Promise.all([
      loadAdminDraftQueueFromBackend({ silent: true }),
      loadAdminInviteLeadsFromBackend({ silent: true })
    ]);
    renderAdminDrafts();
  } catch (e) {
    console.warn('[mitmi] createDraftFromInviteLead:', e.message);
    showToast('Signal iz poziva trenutno nije poslat u draftove', 'error');
  }
}

function renderAdminInviteLeads() {
  syncAdminUI();
  const list = document.getElementById('admin-invite-lead-list');
  if (!list) return;
  if (!ADMIN_INVITE_LEADS.length) {
    list.innerHTML = '<div class="draft-empty">Za sada nema poziva koji izgledaju kao pravi događaji za katalog.</div>';
    return;
  }
  list.innerHTML = ADMIN_INVITE_LEADS.map(item => {
    const meta = [adminDraftTimeLabel(item.startsAt), item.locationName || item.city || 'Lokacija nije upisana'].filter(Boolean).join(' · ');
    return `<div class="draft-card"><div class="draft-top"><div style="flex:1;min-width:0"><div class="draft-title">${_escHtml(item.eventTitle)}</div><div class="draft-meta">${_escHtml(meta)}</div></div><div class="draft-chip-row" style="justify-content:flex-end"><span class="tag tag-gold">Poziv</span><span class="tag tag-amber">Signal</span></div></div><div class="draft-note"><strong>Objavio/la:</strong> ${_escHtml(item.creatorName)}</div><div class="draft-note" style="margin-top:8px"><strong>Poziv:</strong> ${_escHtml(item.title || 'Tražim društvo')}</div>${item.description ? `<div class="draft-note" style="margin-top:8px">${_escHtml(item.description)}</div>` : ''}<div class="draft-actions"><button class="btn btn-purple btn-sm" onclick="createDraftFromInviteLead('${item.id}')">Pošalji u draftove</button><button class="btn btn-outline btn-sm" onclick="openEventById('${_escHtml(item.eventId || '')}')">Otvori događaj</button></div></div>`;
  }).join('');
}
...
async function createGhostOrganizerForDraft(draftId) {
  ...
  ADMIN_ORGANIZERS.unshift({ id:newId, name:draft.proposedOrganizerName || 'Organizer u pripremi', city:draft.city || 'Novi Sad', instagram:(draft.proposedOrganizerInstagram || '').replace(/^@+/, ''), status:'ghost' });
