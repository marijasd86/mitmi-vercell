    ...
    box.innerHTML = items.map(item => {
      ...
      return `<div class="inv-row">
        ...
        ${isOwn
          ...
          : `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openInviteDirectChat('${safeProfileId}','${safeName}','${safeInviteId}','${safeEventId}','${safeTitle}','${safeEventTitle}','${safeItemId}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToInvite('${safeInviteId}','${safeProfileId}','${safeName}','${safeTitle}','${safeEventId}','${safeEventTitle}','${safeItemId}','${isLegacySource ? 'legacy' : 'plan'}')">${isLegacySource ? 'Prijavi se' : 'Javi se'}</button></div>`}
      </div>`;
    }).join('');
    return items;
  } catch (e) {
...
async function openEventById(eventId) {
  ...
  if (!card && _isSupabaseConfigured()) {
    try {
      const rows = await _supaGet('events', {
        select: 'id,creator_id,venue_id,organizer_id,title,description,category,city,location_name,organizer_name_override,starts_at,capacity,attendee_count,cover_url,avg_rating,rating_count,is_published,is_cancelled,created_at',
        id: `eq.${eventId}`,
        limit: '1'
      });
      const row = Array.isArray(rows) ? rows[0] : null;
