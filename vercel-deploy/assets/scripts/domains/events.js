    ...
    box.innerHTML = items.map(item => {
      ...
      return `<div class="inv-row">
        ...
        ${isOwn
          ...
          : `<div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openInviteDirectChat('${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}','${_escHtml(actionInviteId)}','${_escHtml(item.event_id || '')}','${_escHtml(item.title || 'Plan').replace(/'/g, '&#39;')}','${_escHtml(eventTitle).replace(/'/g, '&#39;')}','${_escHtml(item.id || '')}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToInvite('${_escHtml(actionInviteId)}','${_escHtml(profile.id || '')}','${_escHtml(name).replace(/'/g, '&#39;')}','${_escHtml(item.title || 'Plan')}','${_escHtml(item.event_id || '')}','${_escHtml(eventTitle).replace(/'/g, '&#39;')}','${_escHtml(item.id || '')}','${isLegacySource ? 'legacy' : 'plan'}')">${isLegacySource ? 'Prijavi se' : 'Javi se'}</button></div>`}
      </div>`;
    }).join('');
    return items;
  } catch (e) {
  ...
  try {
    ...
    box.innerHTML = items.map(item => {
      ...
      const actionButtons = isOwn
        ...
        : eventId
          ? `<button class="btn btn-outline btn-sm" onclick="event.stopPropagation();openInviteDirectChat('${_escHtml(profile.id || '')}','${safeName}','${isLegacySource ? _escHtml(item.id) : ''}','${_escHtml(eventId)}','${safeTitle}','${safeEventTitle}','${_escHtml(item.id || '')}')">Poruka</button><button class="btn btn-purple btn-sm" onclick="event.stopPropagation();applyToInvite('${isLegacySource ? _escHtml(item.id) : ''}','${_escHtml(profile.id || '')}','${safeName}','${safeTitle}','${_escHtml(eventId)}','${safeEventTitle}','${_escHtml(item.id || '')}','${isLegacySource ? 'legacy' : 'plan'}')">${isLegacySource ? 'Prijavi se' : 'Javi se'}</button>`
...
async function applyToInvite(inviteId, creatorId, creatorName = 'mitmi korisnik', inviteTitle = 'Poziv', eventId = '', eventTitle = 'Događaj', planId = '', sourceType = 'legacy') {
  if (!isLoggedIn()) {
  ...
  if (normalizedSource !== 'legacy' || !inviteId) {
    showToast('Otvaramo poruke da se dogovorite oko izlaska', 'success', 1600);
    await openInviteDirectChat(creatorId, creatorName, '', eventId, inviteTitle, eventTitle, planId || '');
    return;
  }
  try {
...
function _formatEventDateTimeLine(event = {}) {
  ...
  if (!parsed) return 'Vreme nije upisano';
...
function renderBrowseEventsGrid() {
  ...
  grid.innerHTML = items.map(event => {
    const categoryLabel = _catLabel(event.cat || 'drugo');
...
async function loadMyInvites() {
  ...
  try {
    ...
    const legacyInviteIds = plans.filter(item => _isLegacyInviteItem(item) && item.event_id && item.events).map(item => item.id).filter(Boolean);
    let counts = new Map();
